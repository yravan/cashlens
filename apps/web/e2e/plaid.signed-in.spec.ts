...
test.describe("plaid connect flow (real sandbox)", () => {
  ...
  async function credentialedConnections(): Promise<Array<{ id: string; status: string }>> {
  ...
  async function cleanup() {
    const credentialed = await credentialedConnections();
  ...
  async function disconnectOwned(page: Page) {
    const credentialed = await credentialedConnections();
