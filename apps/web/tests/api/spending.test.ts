...
test("a period narrows every number and disclosure count exactly", async () => {
  ...
  const streaming = (value: number) => {
    const netMinor = BigInt(value);
    return ({
    ...
    netMinor,
    categories: [
      { id: demoCategory("Streaming & Music", "leaf"), name: "Streaming & Music", spentMinor: netMinor, receivedMinor: BigInt(0), netMinor },
    ],
    });
  };
  const paycheck = (value: number) => {
    const netMinor = BigInt(value);
    return ({
    ...
    netMinor,
    categories: [
      { id: demoCategory("Paycheck", "leaf"), name: "Paycheck", spentMinor: BigInt(0), receivedMinor: netMinor, netMinor },
    ],
    });
  };
  ...
  expect(march.currencies).toEqual([
    ...
    {
      currency: "USD",
      totals: { spentMinor: BigInt(-15279), receivedMinor: BigInt(272112), netMinor: BigInt(256833) },
      groups: [EXPECTED.demo.spending[1].groups[0], streaming(-2300), paycheck(250000)],
      uncategorized: { spentMinor: BigInt(-1800), receivedMinor: BigInt(22112), netMinor: BigInt(20312) },
    },
  ]);
  ...
  expect(february.currencies).toEqual([
    {
      currency: "USD",
      totals: { spentMinor: BigInt(-21299), receivedMinor: BigInt(250000), netMinor: BigInt(228701) },
      groups: [streaming(-2300), paycheck(250000)],
      uncategorized: { spentMinor: BigInt(-18999), receivedMinor: BigInt(0), netMinor: BigInt(-18999) },
    },
  ]);
...
test("exclusion is pair membership: unmatched legs sit in uncategorized, and net is invariant", async () => {
  ...
  expect(usd(before).uncategorized).toEqual({
    spentMinor: BigInt(-225799),
    receivedMinor: BigInt(227112),
    netMinor: BigInt(1313),
  });
  expect(usd(before).totals).toEqual({
    spentMinor: BigInt(-243878),
    receivedMinor: BigInt(977112),
    netMinor: BigInt(733234),
  });
...
test("rollup is leaf arithmetic: recategorizing moves leaves between groups, totals invariant", async () => {
  ...
  expect(usd(merged).groups[0]).toEqual({
    ...
    categories: [
      { id: demoCategory("Groceries", "leaf"), name: "Groceries", spentMinor: BigInt(-11179), receivedMinor: BigInt(0), netMinor: BigInt(-11179) },
    ],
  });
  ...
  expect(usd(moved).groups.map((group) => [group.name, group.netMinor])).toEqual([
    ["Entertainment", BigInt(-11337)],
    ["Food & Drink", BigInt(-6742)],
    ["Income", BigInt(750000)],
  ]);
  expect(usd(moved).groups[0].categories).toEqual([
    { id: demoCategory("Streaming & Music", "leaf"), name: "Streaming & Music", spentMinor: BigInt(-6900), receivedMinor: BigInt(0), netMinor: BigInt(-6900) },
    { id: demoCategory("Hobbies", "leaf"), name: "Hobbies", spentMinor: BigInt(-4437), receivedMinor: BigInt(0), netMinor: BigInt(-4437) },
  ]);
...
test("a pending row is disclosed, not counted, and joins its category when it posts", async () => {
  ...
  expect(usd(summary).groups[0]).toEqual({
    id: demoCategory("Food & Drink", "group"),
    name: "Food & Drink",
    spentMinor: BigInt(-12429),
    receivedMinor: BigInt(0),
    netMinor: BigInt(-12429),
    categories: [
      { id: demoCategory("Groceries", "leaf"), name: "Groceries", spentMinor: BigInt(-6742), receivedMinor: BigInt(0), netMinor: BigInt(-6742) },
      { id: demoCategory("Restaurants & Bars", "leaf"), name: "Restaurants & Bars", spentMinor: BigInt(-4437), receivedMinor: BigInt(0), netMinor: BigInt(-4437) },
      { id: demoCategory("Coffee Shops", "leaf"), name: "Coffee Shops", spentMinor: BigInt(-1250), receivedMinor: BigInt(0), netMinor: BigInt(-1250) },
    ],
  });
  expect(usd(summary).totals).toEqual({
    spentMinor: BigInt(-40128),
    receivedMinor: BigInt(772112),
    netMinor: BigInt(731984),
  });
});
