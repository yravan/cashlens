...
15. **7.1** In-app notification feed
...
- **1.3 Hosting & delivery** `[MVP]`
  - **1.3.1 Production deployment** `[MVP]` — The app and its database run at a stable public URL. Merging to main ships automatically.
  - **1.3.2 Preview environments** `[P2]` — Every change can be deployed to a throwaway URL before merge.
...
- **2.2 Manual & file input** `[P1]`
  ...
  - **2.2.3 Offline accounts** `[P1]` — Track accounts with no feed (cash, a 401k, niche platforms) by periodically updating a balance.
...
- **2.5 Investments & fragmented money** `[P2]`
  - **2.5.1 Investment accounts** `[P2]` — Connect brokerage and retirement accounts (Fidelity, Robinhood, 401k) for balances and holdings, so net worth is complete.
  - **2.5.2 Niche platforms** `[P3]` — Kalshi and friends, via statements or manual balance updates.
...
- **3.3 Deduplication & matching** `[P1]`

  *One real-world event appears exactly once, enriched by every source that saw it.*
  - **3.3.1 Internal transfer matching** `[P1]` — Pair moves between your own accounts — credit-card payments, savings transfers — so flow numbers never double-count them.
...
- **3.5 Expected & future transactions** `[P1]`
  ...
  - **3.5.2 Expectation matching** `[P2]` — When an expected charge or refund arrives, link it automatically; alert when it's overdue or the amount is wrong.
...
- **4.2 Automatic categorization** `[MVP]`
  - **4.2.1 LLM auto-categorization** `[MVP]` — Every uncategorized transaction — new or backfilled — gets a category, a confidence, and a one-line reason from an LLM. Low confidence goes to review.
...
- **4.3 Merchant enrichment** `[P2]` — Ugly bank strings resolve to clean merchant names and logos, shared across all of a merchant's transactions.
- **4.4 Line-item categorization** `[P2]` — Receipt line items get their own categories, so one Costco run truthfully splits into groceries versus household. Aggregations use items when present.
...
- **6.4 Recurring & upcoming** `[MVP]`
  ...
  - **6.4.2 Upcoming expenses view** `[MVP]` — Calendar and list of predicted next charges from recurring patterns plus scheduled obligations, with the expected total still to leave this month.
...
- **6.5 Surprises & anomalies** `[P2]`
  - **6.5.1 Unexpected charge detection** `[P2]` — First-time merchants, resumed subscriptions, off-schedule or duplicate charges flagged as they arrive.
  - **6.5.2 Amount anomalies** `[P2]` — Charges that deviate from their own history — the UPS overcharge — flagged with the comparison.
...
- **6.7 Taxes** `[P3]`
  - **6.7.1 Tax payments view** `[P3]` — Every tax payment and refund across accounts in one place, with year totals and surplus or deficit.
  - **6.7.2 Deductible flagging** `[P3]` — Tag potentially deductible spending all year; export a year-end summary.
- **6.8 Export & digest** `[P2]`
  - **6.8.1 Data export** `[P2]` — Full transactions and categories out as CSV or JSON. It's your data.
  - **6.8.2 Periodic digest** `[P2]` — A weekly or monthly summary of flows, anomalies, and upcoming bills, delivered through notification channels.

## 7. Notifications & Alerts

*The app reaches out when something happens — never noisily.*

- **7.1 In-app notification feed** `[MVP]` — Notification center with an unread badge: new transactions and connection problems. Mark read, or read all.
...
- **7.4 Email notifications** `[P2]` — Chosen events and digests, by email.
...
- **MVP notifications are in-app only** — push (web and iOS) is P2.
- **iOS is a P2 branch** — "website and iOS app" is the vision, but the web alone delivers the MVP.
- **The wedge features are all P1** — manual entry, Gmail receipts, Venmo, splits, reimbursements, returns, review queue: the first wave after MVP, in roughly the order listed.
