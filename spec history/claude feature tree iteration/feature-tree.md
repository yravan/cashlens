...
Version: **v8 (final)** · See [00-README.md](00-README.md) for vision, conventions, legend, and the iteration protocol.
Leaf format: `**ID — Title** [status] size layer — scope.`
followed by an optional compact sub-line of `_deps:_ … · _skill:_ … · _data:_ … · _accept:_ … · _test:_ … · _xc:_ … · _perf:_ … · _benchmark-by:_ … · _risk:_ …`
Status `[built]`/`[partial]`/`[new]` · size `S`/`M`/`L` · layer `api·web·ios·infra·data·shared·research`.

Sub-field legend (added v1):
- `_deps:_` upstream leaf IDs that must land first. **(v2: pruned to direct/non-transitive edges; graph verified acyclic — 0 cycles, 0 dangling. v3: re-verified after PLT-9.1/PLT-4.6 edges. v4: re-verified after ENR-2.7 + SRC-1.5. v5: re-verified after new SRC-1.7 SimpleFIN adapter + ING-6.1 write-path-budget edges. v6: re-verified after new ING-6.2 burst/back-pressure budget edges — 347 nodes, acyclic. v7: re-verified after 2 new edges (ING-6.2→PLT-8.4 burst-test; PLT-2.3→PLT-2.1 so the replay harness follows the framework it exercises) — 346 nodes, 360 edges, 0 cycles/dangling/self-loops; the build-order block E + the corrected critical path were reconciled against this graph so no leaf is scheduled before a blocker. v8 (final): re-verified after the 1 risk→dep promotion `SEC-3.1→SEC-4.1` (irreversible account erasure must not ship before the append-only audit log its own `_risk:_` says to write to — see risk→dep sweep below) — 346 nodes, **361 edges**, 0 cycles/dangling/self-loops. The other 11 risk-cited mitigations were evaluated and kept ADVISORY: 5 would create a cycle because the mitigation is a forward/UI leaf that already deps the destructive one (`SEC-3.1→AUT-4.2`, `ENR-3.1→ENR-3.2/3.3`, `LDG-4.3→LDG-4.4`), and the rest are "X-style"/fallback references, not hard ordering constraints. ALL header ordering claims were re-validated against the graph; the v7 "auth chain `SEC-1.1→SEC-1.2→AUT-3.1`" was DROPPED as a non-edge — `AUT-3.1` deps `SEC-1.1` directly, not `SEC-1.2` (leaf wins).)**
- `_flag:_` (v8) the PLT-5.3 feature-flag key (from index-block B) that gates this leaf, on every flag-gated leaf — so the leaf names its key instead of the catalog being the only place the relationship is recorded. **(v8: added to the 9 owning leaves block B lists — AUT-5.2, SRC-4.1, SRC-5.1, SRC-6.1, SRC-7.1, CAT-4.1, NTF-3.1, NTF-4.1, IOS-1.1; SRC-1.7 already carried it. Closes the iteration-07 block-B drift where 8 owning leaves named no key.)**
...
- `_files:_` likely repo files/dirs the leaf touches, inferred from layout (`apps/api/src/cash_lens_api/{models.py,schemas.py,routers/,services/,core/}`, `apps/web/...`, `packages/api-types/src/index.ts`, `apps/ios/...`). Guidance, not a contract. **(v3: 80 leaves. v4: 140 leaves — web/INT/OWE/SRC. v5: ~200 leaves — finished RCP receipt/OCR services (`services/receipts.py`, `services/ocr.py`) + RPT chart components (`apps/web/components/charts/*`) + remaining CAT/SEC/AUT/PLT-infra. Verified real web names: `summary-card.tsx` (singular), `server-api.ts`/`session.ts`/`types.ts`/`utils.ts`/`runtime.ts` under `apps/web/lib/`; `charts/` + `command-palette.tsx` + `filter-panel.tsx` are forward-looking new dirs/files.)** **(v6: closed iteration-05 self-critique #4 — paths are now disambiguated by the §"Real vs new file paths" block: files that already exist in the repo (verified 2026-05-29 against `routers/{accounts,dashboard,health,notifications,plaid,transactions,users}.py`, `services/{dashboard,demo_seed,plaid}.py`, `core/{auth,config,security}.py`, `apps/web/lib/{server-api,session,types,utils,runtime}.ts`, `apps/web/components/{app-shell,summary-card,...}.tsx`) vs forward-looking NEW paths an agent will create. An implementer treats a NEW path as a file to author, a real path as one to edit.)**
- `_accept:_` one-line acceptance criterion (the observable that proves it works).
- `_test:_` proving test layer — `pytest` (api), `vitest` (web unit), `playwright` (e2e), or `manual`/`doc`. **(v2: wedge leaves cite a named golden fixture + invariant. v3: reports/forecast/budgets/returns/net-worth/FX. v4: edge fixtures `GF-FORECAST-PAUSE/GF-BUDGET-ROLLOVER/GF-FX-TRI`. v5: added *combination-branch* fixtures `GF-COMBO-SRR` (split that is also reimbursable + partly a return) and `GF-COMBO-DUPXFER` (a true double-charge that superficially looks like a transfer pair) to harden where two wedge rules collide. v6: `GF-COMBO-STACK` (dedup→split→reimburse stacked) + `GF-DEDUP-XPROV` (same bank txn from two providers). v7: `GF-COMBO-FX` — a triangulated FX conversion INSIDE a split INSIDE a dedup group (the hardest unmodeled collision; native-currency split math + report-time conversion, no rounding loss).)**
- `_xc:_` cross-cutting reqs honored: `authz` (server-verified row-scoped identity), `crypto` (encrypted at rest), `idem` (idempotent/replay-safe), `rate` (rate-limited), `a11y`, `money` (integer minor units), `obs` (observability/metrics). **(v4: every `obs` leaf names the exact PLT-4.6 contract signal it emits, in-line as `obs:<signal>`.)** **(v5: disambiguated the overloaded `obs:request_id` (iteration-04 self-critique #2) — a metric-emitter tags its histogram/counter (e.g. `obs:http.server.duration`); a leaf whose deliverable is durable data or a log/error scrub (not a metric) now tags `obs:request_id(durable)` to show it carries the correlation id WITHOUT inventing a metric name. PLT-4.1 alone tags bare `obs:request_id` because it DEFINES the id.)** **(v6 cross-cutting completeness sweep: audited every leaf for the reqs it MUST honor — added `money` to money-display leaves that lacked it (OWE-3.2 expected-refund badge, RPT-3.2 drilldown amounts, RPT-3.3 health rollup), `a11y` confirmed on all *interactive* web/iOS leaves (the 6 web/iOS leaves without it are non-interactive: middleware AUT-2.3, client/refresh AUT-2.4/IOS-1.4, background instrumentation PLT-4.3, demo toggle AUT-1.4, APNs registration NTF-4.1 — flagged here so their absence is intentional, not a gap), and verified every `idem`/`authz`/`rate`/`crypto`/`obs` tag is present where the deliverable demands it. The cross-cutting matrix is no longer siloed in SEC/PLT.)**
- `_risk:_` only on risky/ambiguous/spike leaves — risk + one-line mitigation. **(v2: every `research` spike now carries `_cost:_` $ estimate + `_decide-by:_` exit criterion/timebox.)** **(v7 risk-completeness sweep: every DESTRUCTIVE leaf now carries a `_risk:_` — added to account-deletion SEC-3.1, the cascade-FK migration SEC-3.5, dedup/merge ENR-3.1 + unmerge ENR-3.3, the money read-cutover LDG-4.4, the demo→real wipe ONB-4.1, and category-merge CAT-1.4. The existing money-migration LDG-4.3 + provider/FX/LLM risks are retained. Destructive = irreversibly deletes/collapses/reassigns rows or drops a representation.)**
...
- `_benchmark-by:_` (v7) — on every `_perf:_` number classified **HYP** in index-block A (a hypothesis-to-benchmark, not yet a contract): the validation trigger/deadline by which it must be measured in a real load run (PLT-8.4) and either promoted to **SLO** or re-set. Mirrors how a `research` spike carries `_decide-by:_` — it stops a guessed budget from staying unvalidated forever (closes iteration-06 self-critique #5). The 8 HYP leaves (PLT-8.2, PLT-8.4, ING-6.1, ING-6.2, LDG-2.5, CAT-2.1, CAT-2.2, CAT-4.2) each carry one; SLO leaves do not (their budget already has an index/EXPLAIN basis).
...
> - `GF-COMBO-SRR` (v5) — **split + reimbursable + return on one $120 group-dinner parent** (proves the three don't cross-wire). Split 4×$30; payer keeps 1, 2 marked reimbursable (Venmo-back), payer's own $30 is to-return. Invariants (all simultaneous): (a) splits sum to $120 (`GF-SPLIT`); (b) true-spend = payer's net non-reimbursed/non-returned = $30 while the return is open, $0 once the 2 receivables settle + the return refunds; (c) one $30 Venmo closes exactly one receivable, not the return; (d) the −$30 refund closes the return, not a receivable; (e) `GF-REPLAY` preserves all three sub-states.
> - `GF-COMBO-DUPXFER` (v5) — **a true double-charge that looks like a transfer pair / dedup candidate** (proves transfer-pairing, dedup, and dup-charge detection stay mutually exclusive). Same merchant charges a card −$60 twice in 2 days (genuine duplicate billing); both legs are the SAME direction (outflow) on the SAME account. Invariants: (a) NOT a transfer pair (needs opposite `direction`, `GF-TRANSFER`); (b) NOT collapsed by cross-source dedup (needs distinct sources, `GF-DEDUP`); (c) ARE flagged as a double-charge (INT-4.2); (d) both count toward true-spend until the user confirms one as error/return.
> - `GF-COMBO-STACK` (v6) — **stacked chain: a deduped multi-source parent that is THEN split + reimbursed** (proves dedup→split→reimburse compose without member fan-out). A $96 dinner via **bank + Venmo + email receipt** (`GF-DEDUP`) → 1 canonical $96 (bank by precedence) → split 3×$32 (`GF-SPLIT`) → 2 shares reimbursable. Invariants (in order): (a) the 3 source rows = exactly **1** `purchase_group`; the split attaches to the **canonical** member only (never a non-canonical source row); (b) Σ(3 splits)=$96 against the *canonical* amount, not 3×$96; (c) 2 reimbursable shares → exactly **2** receivables of $32, not 6; (d) true-spend = payer's net **$32 once**, never $288 (3 sources × 3 splits); (e) `GF-REPLAY` preserves the group + splits + receivables, zero row delta.
> - `GF-DEDUP-XPROV` (v6) — **cross-PROVIDER dedup variant of `GF-DEDUP`** (Plaid + the SRC-1.7 SimpleFIN adapter cover the same institution → the same bank txn arrives once from each). A −$73.40 grocery charge ingested as a `provider='plaid'` row AND a `provider='simplefin'` row (same institution/account/date/amount). Invariants: (a) the two are DISTINCT raw rows (different `provider`/`provider_ref` → ING-4.1 provider-id idempotency does NOT collapse them); (b) cross-source matching (ENR-2.1/2.6) scores them ≥ auto-link band → dedup (ENR-3.1) → **1** canonical, counted **once** (`GF-DEDUP` generalizes across providers); (c) provider tie-break is deterministic (Plaid > SimpleFIN, extending bank>card>Venmo>receipt); (d) stays distinct from `GF-COMBO-DUPXFER` — two SimpleFIN-only same-account same-direction charges are a double-charge, not a cross-provider dup (which requires *different* providers).
> - `GF-COMBO-FX` (v7) — **the hardest collision: triangulated FX INSIDE a split INSIDE a dedup group** (proves dedup + FX-triangulation + split + reimburse compose with no member fan-out, no rounding loss, no destructive currency rewrite). Primary USD. A **£90 London dinner** via **bank + card foreign-txn row + email receipt** (`GF-DEDUP`-style); on the date only **EUR↔USD 1.10** + **GBP↔EUR 1.17** exist (no direct GBP→USD → `GF-FX-TRI`: £90 → €105.30 → $115.83); then split 3 ways, 2 shares reimbursable (`GF-COMBO-STACK`-style). Invariants (in order): (a) the 3 sources = **1** `purchase_group`; the canonical stays **9000 GBP-minor** — conversion is a *report-time rollup*, never a stored-currency rewrite (`GF-FX`/`LDG-4.1`); (b) the split sums in **native** currency (Σ 3 splits == **9000 GBP-minor exactly**, no float drift) — NOT in converted USD, since splitting the rounded $115.83 would lose a cent; convert each share for display only; (c) the USD rollup uses the triangulated rate once (≈$115.83) and per-share USD sums back to it within **±1-minor-unit** — convert-then-sum == sum-then-convert to tolerance, residual cent owned by the last share, never dropped/duplicated; (d) 2 reimbursable shares → exactly **2** GBP-denominated receivables (settled by GBP-equivalent inflows), not 6; (e) a missing-rate day triangulates or flags unconvertible (never silent 1:1), dedup unaffected; (f) `GF-REPLAY` preserves the group, GBP-stored splits, receivables, and cached rate, zero row delta.
>
...
> **First-12-PR starter slice (dispatch order — see the dispatch table in index-block E for the full topo-verified order):** PLT-2.1 → PLT-2.3 → SEC-1.1 → SEC-1.3 → SEC-1.2 → LDG-4.3 → SRC-1.2 → CAT-1.1 → ENR-2.7 → ENR-2.1 → REV-1.1 → PLT-2.2. **(v7: corrected so the slice is internally dependency-consistent — `LDG-4.3` deps `PLT-2.3` (the replay harness it needs to prove the float→int backfill is re-runnable), so PLT-2.3 now precedes it; `ENR-2.1` deps `ENR-2.7` (match-config owner), so ENR-2.7 now precedes it. The v6 slice listed LDG-4.3 before PLT-2.3 and ENR-2.1 before ENR-2.7 — both scheduled before a blocker.)** Rationale: migrations + the replay harness + server-verified auth + encrypted tokens + integer money are the foundation every data leaf depends on; taxonomy + match-config + candidate-gen + queue unlock the wedge.
> **Critical path (longest dependency chain to deduped true-spend, the wedge):** PLT-2.1 → PLT-2.3 → LDG-4.3 → ENR-2.1 → ENR-2.6 → ENR-3.1 → ENR-3.4 (deduped true-spend). **(v7 correction: the v6 chain claimed `LDG-4.3 → LDG-4.4 → ENR-2.7 → ENR-2.1`, but `LDG-4.4` (the read cutover) is NOT on the path to dedup — `ENR-2.1`/`ENR-3.x` depend only on `LDG-4.3` (the minor-unit schema+backfill), not the read cutover — and `ENR-2.7` depends only on `PLT-2.1`, NOT on `LDG-4.4`. The real prerequisites of `ENR-2.1` are `ENR-2.7` + `LDG-4.3`, which join here. Verified against the dependency graph.)** `ENR-2.7` (match-config) is a parallel prerequisite that must land before `ENR-2.1`. Parallel foundation chain: PLT-2.1 → SEC-3.5/SRC-1.2 (both real edges); auth chain: SEC-1.1 → {SEC-1.2, AUT-3.1} (**v8 correction: SEC-1.2 and AUT-3.1 BOTH dep SEC-1.1 directly and run in parallel after it — the v6/v7 "SEC-1.1 → SEC-1.2 → AUT-3.1" implied AUT-3.1 deps SEC-1.2, which is NOT a real edge; verified against the graph, leaf wins**); longest absolute chain (incl. perf budgets): PLT-2.1 → ENR-2.7 → ENR-2.1 → ENR-2.6 → ENR-3.1 → ING-6.1 → ING-6.2 (length 6, every link a real edge). **(v4: SRC-1.5 provider spike RESOLVED — default Plaid (shipped), SimpleFIN documented fallback. v5: the fallback is now ACTIONABLE — SRC-1.7 SimpleFIN adapter conforms to the SRC-1.1 interface behind a flag, so swapping/adding the low-cost provider is a coded path, not just a recommendation. Remaining gating spikes: AUT-5.1, IOS-1.1.)**
...
> **Phase rollup:** P0 Foundation (PLT-2 incl. PLT-2.3 replay harness, PLT-9.1, SEC-1, LDG-4.3/4.4, AUT-1/2, AUT-5.1 spike) · P1 Source+Ingest (SRC-1/2, ING-1/2/3) + PLT-4.6 obs-contract · P2 Ledger+Cat (LDG-1/2/3, CAT-1/3/4) · P3 Wedge (ENR, RCP, OWE) · P4 Intelligence+Reports (INT, RPT) · P5 Review+Nav+Notif (REV, NAV, NTF) · P6 iOS (IOS). Cross-phase: PLT-8 (read perf) + ING-6 (write-path perf, v5; ING-6.2 sync-burst/back-pressure budget, v6) span P1–P4. **(v7: the HYP perf budgets in PLT-8/ING-6 carry a `_benchmark-by:_` gate keyed to the **first PLT-8.4 load run at the P3→P4 boundary** — that run, now extended to a burst/storm mode (PLT-8.4), is where every HYP number is measured and either promoted to SLO or re-set. The dispatch order is enumerated in index-block E.)**

---

## Index blocks (navigability — reference tables, NOT leaves)

> These blocks (A perf budgets · B feature flags · C golden fixtures · D file-path convention · E build-order/dispatch · **F vision→leaf coverage matrix, v8**) collect numbers/keys/fixtures/order/coverage that were previously scattered across leaves, so a reviewer can scan them in one place. They restate, never replace, the owning leaf — the leaf remains canonical.
>
> **Consistency rule (v7, closes iteration-06 self-critique #1):** these tables are a **regenerated snapshot, not a hand-maintained second source.** When a pass edits a leaf's `_perf:_`, `_risk:_`, flag key, fixture, or `_deps:_`, it regenerates blocks A–E from the leaves at the END of the pass (a `verify` step diffs them). Treat any disagreement between a table and its owning leaf as a regen-lag bug to fix in that pass, with the **leaf winning**. Blocks B (flags via PLT-5.3) and the match-config (ENR-2.7) additionally have a *runtime* owner that seeds the real data; A/C/D/E are documentation snapshots only.
>
> **v8 (final) — the regen-and-diff was actually EXECUTED (closes iteration-07 open question #1).** Every leaf's `_perf:_`/`_benchmark-by:_`/`GF-*` citations and `_deps:_` edges were parsed programmatically and diffed against blocks A/C/E: **block A matches exactly** (the 14 `_perf:_` leaves == the table rows; HYP⇔carries-`_benchmark-by:_`, SLO⇔doesn't, with 8 HYP / 6 SLO); **block E waves, the first-12 slice, the critical path, and the longest chain are all real edge chains** (no leaf precedes a blocker). Two drifts were fixed, **leaf-as-truth**: (1) **block B** named 10 flag keys but 8 owning leaves never cited their key — each owning leaf now carries an explicit `flag: <key> (block B)` tag so leaf and catalog agree; (2) **block C** primary-leaf lists were extended to include the leaves that genuinely exercise a fixture but were omitted (`GF-DEDUP` += ENR-3.4/SRC-1.7; `GF-TRUESPEND` += RPT-2.2). The **vision→leaf coverage matrix** (block F, below) is likewise regenerated from leaf bodies — every README §1 bullet + north-star question maps to ≥1 leaf with no holes.

### A. Performance-budget index (every `_perf:_` target in one table)

Each number is tagged **SLO** (a contract the implementer must hold — derived from a read-tier budget that has an EXPLAIN/index basis) or **HYP** (*hypothesis-to-benchmark* — a reasoned target to validate in the first PLT-8.4 perf run, NOT yet a contract; iteration-05 self-critique #2). Tier: R=read, W=write. **Bench-by (v7):** every HYP row now carries a `_benchmark-by:_` deadline on its leaf (a validation trigger), so a guess can't stay unvalidated forever (closes iteration-06 self-critique #5). SLO rows show "—" (their budget already has an index/EXPLAIN basis verifiable at seed time, no separate gate).

| Leaf | Tier | Budget | Class | Bench-by |
|------|------|--------|-------|----------|
| PLT-8.1 | R | txn page (100 rows) p95 < 150ms @100k; index-only keyset | **SLO** | — |
| PLT-8.2 | W | backfill ≥ 5,000 txns/min; batch 500; RSS Δ < 256 MB @50k | **HYP** (throughput/memory unmeasured) | first PLT-8.4 run (P1, before ING-1.x ships to prod) |
| PLT-8.3 | R | monthly group-by p95 < 300ms cold / < 50ms warm @100k; index scan | **SLO** (cold; warm depends on cache hit) | — |
| PLT-8.5 | R | `GET /dashboard` p95 < 400ms cold / < 80ms warm @100k | **SLO** (cold) | — |
| PLT-8.4 | — | seed 100k txns < 5 min on CI; **+ burst mode (v7)** | **HYP** (CI-hardware dependent) | self (this leaf IS the load harness; P0/P1) |
| ING-6.1 | W | write chain ≥ 2,000 txns/min e2e; per-100 batch p95 < 800ms; bounded fan-out | **HYP** (reasoned from read tier, unmeasured) | first PLT-8.4 run at P3→P4 boundary |
| ING-6.2 | W | ≤ 8 in-flight sync jobs/instance; queue depth alert > 500; shed/defer on breach | **HYP** (burst load unmodeled — validate in load test) | first PLT-8.4 **burst-mode** run, P3→P4 boundary |
| LDG-2.5 | W | exclude-toggle PATCH p95 < 200ms @100k (flag flip + cache-version bump) | **HYP** | first PLT-8.4 run at P3→P4 boundary |
| CAT-2.1 | W | single category PATCH p95 < 200ms @100k (+ cache invalidation) | **HYP** | first PLT-8.4 run at P3→P4 boundary |
| CAT-2.2 | W | bulk PATCH 500 ids p95 < 1s (one multi-row UPDATE + one invalidation) | **HYP** | first PLT-8.4 run at P3→P4 boundary |
| CAT-4.2 | W | batch-categorize ≥ 300 events/min warm; checkpoint every 200 | **HYP** (LLM-latency dependent) | first batch-categorize run on real LLM (P2/P4) |
| RPT-2.2 | R | Sankey build p95 < 350ms cold / < 60ms warm @100k | **SLO** (cold) | — |
| REV-1.1 | R | queue assembly p95 < 250ms @100k (partial-index scan) | **SLO** | — |
| NAV-3.3 | R | FTS query p95 < 200ms @100k (GIN/trigram; no seq scan) | **SLO** | — |

> **Read-tier = SLO** (it has an index/EXPLAIN basis verifiable at seed time); **write-tier + memory/throughput = HYP** until the first PLT-8.4 perf run measures them. An implementer must NOT treat a HYP number as a hard gate — record the measured value and promote to SLO once benchmarked.

### B. Feature-flag catalog (every `PLT-5.3` flag key, default, owning leaf)

> Closes iteration-05 self-critique #6 — flags now have a single registry like ENR-2.7 owns match-config. **PLT-5.3 owns this table as seeded data** (the `feature_flags` config); every flag-gated leaf reads its key from here, no ad-hoc flag strings. Default OFF = ships dark / behind verification. **(v8 reconciliation: each owning leaf below now carries an explicit `flag: <key> (block B)` tag in its body — v7 had 8 owning leaves that named no key, so leaf and catalog now agree per the consistency rule.)**

| Flag key | Default | Owning leaf | Gates |
|----------|---------|-------------|-------|
| `provider.simplefin` | OFF | SRC-1.7 | SimpleFIN adapter + its routes (404 when off) |
| `source.gmail` | OFF | SRC-6.1 | Gmail OAuth + receipt import (blocked on CASA verification) |
| `source.sms_imessage` | OFF | SRC-7.1 | iMessage receipt ingestion (spike-gated; forward-to-email is the live path) |
| `source.paypal` | OFF | SRC-4.1 | PayPal import (spike-gated) |
| `investments` | OFF | SRC-5.1 | Investment holdings sync (Plaid Investments billed per call) |
| `household` | OFF | AUT-5.2 | Household/shared-access surfaces (default = personal only) |
| `llm.categorize` | OFF | CAT-4.1 | LLM categorization (gated on SEC-5.2 no-train policy) |
| `notif.web_push` | OFF | NTF-3.1 | Web push (VAPID) registration + send |
| `notif.ios_push` | OFF | NTF-4.1 | iOS APNs push |
| `ios.app` | OFF | IOS-1.1 | iOS client surfaces (stack-spike gated) |

> Flag keys are dotted `domain.feature`. Adding a flag = add a row here + reference it from the owning leaf's `_data:_`; PLT-5.3 seeds defaults idempotently (re-seed = no dup, per `GF-REPLAY`).

### C. Golden-fixture catalog (terse index; full definitions in the header block above)

| Fixture | One-line invariant | Primary leaves |
|---------|--------------------|----------------|
| `GF-DEDUP` | 1 purchase via 3 sources → counted once | ENR-2.1/2.6/3.1/3.4, INT-4.2, SRC-1.7 |
| `GF-TRANSFER` | opposite-leg pair excluded from true-spend | LDG-2.2/2.6 |
| `GF-CARDPAY` | card payment excluded; underlying purchases kept | LDG-2.3 |
| `GF-TRUESPEND` | true-spend = Σout − transfers − cardpay − excl − refunds | LDG-2.4, RPT-2.2/2.4/3.1, RPT-5.3 |
| `GF-RECURRING` | 13 charges → 1 stream; price-hike flagged | INT-1.1/1.2/1.5, INT-4.3 |
| `GF-SPLIT` | child splits sum to parent; receivables created | ENR-4.1, OWE-1.3/2.1 |
| `GF-MONEY` | float→integer-minor backfill exact & replay-safe | LDG-4.3 |
| `GF-REPLAY` | re-run ingest/derivation/backfill → 0 row delta | every idem leaf |
| `GF-BUDGET` | over-budget alert fires exactly once/period | RPT-6.1/6.2/6.4 |
| `GF-FORECAST` | deterministic EoM projection from streams | INT-3.1/3.2/3.4, INT-3.3 |
| `GF-RETURN` | exact refund closes return; near-miss stays open | OWE-3.1/3.3 |
| `GF-NETWORTH` | assets − liabilities; 1 snapshot/day | LDG-3.2/3.3, RPT-4.2 |
| `GF-FX` | per-currency store; missing-rate ≠ silent 1:1 | LDG-4.1/4.2 |
| `GF-FORECAST-PAUSE` | paused stream drops future charge | INT-3.1/3.3 |
| `GF-BUDGET-ROLLOVER` | rollover is a per-budget flag, not global | RPT-6.1/6.4 |
| `GF-FX-TRI` | triangulated conversion; no-path → flagged | LDG-4.2 |
| `GF-COMBO-SRR` | split + reimbursable + return don't cross-wire | ENR-4.3, LDG-2.4 |
| `GF-COMBO-DUPXFER` | double-charge ≠ transfer ≠ cross-source dup | LDG-2.2, ENR-3.1, INT-4.2 |
| `GF-COMBO-STACK` (v6) | dedup → split → reimburse compose w/o member fan-out | ENR-3.1/4.1/4.3, RCP-3.2, LDG-2.4 |
| `GF-DEDUP-XPROV` (v6) | Plaid + SimpleFIN over same bank → counted once | ENR-2.1/3.1, SRC-1.7 |
| `GF-COMBO-FX` (v7) | FX-triangulation inside split inside dedup; native-currency split, no rounding loss | LDG-4.1/4.2, ENR-3.1/4.1/4.3 |

### D. Real vs new file paths (so `_files:_` says edit-vs-author)

> Closes iteration-05 self-critique #4. `_files:_` paths matching the list below already EXIST (verified 2026-05-29) — an agent EDITS them. Everything else in a `_files:_` line is a forward-looking NEW path the agent AUTHORS, following the repo's layout conventions.

- **api (real):** `routers/{accounts,dashboard,health,notifications,plaid,transactions,users}.py` · `services/{dashboard,demo_seed,plaid}.py` · `core/{auth,config,security}.py` · `main.py` · `db.py` · `models.py` · `schemas.py`
- **web (real):** `apps/web/lib/{server-api,session,types,utils,runtime}.ts` · `apps/web/components/{app-shell,summary-card,plaid-connect-button,manual-sync-button,mark-all-read-button,transaction-editor,user-pill,notification-center,notification-badge,status-badge,section-card,nav-link}.tsx` · `apps/web/app/(app)/{dashboard,accounts,transactions,settings}/` · `apps/web/app/{sign-in,login,layout.tsx,globals.css}`
- **shared (real):** `packages/api-types/src/index.ts`
- **NEW (author):** all `alembic/*` · `core/{tasks,logging,metrics,obs_contract,cache,rate_limit,crypto,secrets,sentry,redaction,audit,flags,blob,ai,scoping}.py` · all `services/{sources/*,ledger/*,enrichment/*,receipts*,ocr,invoices,categorize,rules,recurring,subscriptions,anomaly,forecast,bills,returns,receivables,group_expense,net_worth,reports,review_queue,search,push,notifications,investments,data_export,households,users,budgets,transactions}.py` · all `apps/web/components/{charts/*,dashboard/*,review/*,onboarding/*,command-palette,filter-panel,category-picker,split-editor,split-calculator,rule-editor,match-review,dedup-review,receipt-*,budget-progress,goal-progress,bill-calendar,empty-state,review-progress}.tsx` · all `apps/ios/*`

### E. Build-order / dispatch waves (v7 — topo-verified against the dependency graph)

> Closes iteration-06's BUILD-ORDER mandate: a single dispatch-ordered slice the user can hand to agents in sequence, **verified so no leaf precedes a blocker** (each row's `_deps:_` are all either earlier in this table or already `[built]`/`[partial]`-foundation). Waves group leaves that can run in parallel once the prior wave's blockers land. This is a *starter spine* to the wedge — not every leaf; downstream leaves follow the same `_deps:_` discipline. **Regenerate this block from `_deps:_` whenever an edge changes (consistency rule above).**

| Wave | Leaves (parallel within a wave) | Unlocks |
|------|--------------------------------|---------|
| W0 root | PLT-2.1 · PLT-2.3 · SEC-1.1 · SEC-1.3 · PLT-9.1 · PLT-4.1 · PLT-3.1 | migrations + replay harness + server-auth + encrypted tokens + shared store + obs-id + task runner (all dep-free or partial) |
| W1 | SEC-1.2 · PLT-2.2 · PLT-4.6 · LDG-4.3 · SRC-1.2 · CAT-1.1 · ENR-2.7 | row-scoping, migration CI, obs-contract, **integer money**, neutral `connections`, taxonomy, **match-config owner** |
| W2 | LDG-4.4 · SRC-1.1 · ENR-2.1 · LDG-2.1 · CAT-3.1 | money read-cutover, provider interface, **candidate-gen**, event-type taxonomy, rule model |
| W3 | SRC-1.6 · ENR-2.6 · LDG-2.2 · CAT-3.2 · REV-1.1 | Plaid conformance, **match scoring/links**, transfer pairing, rule-on-ingest, review queue |
| W4 | ENR-3.1 · LDG-2.6 · LDG-2.3 | **cross-source dedup**, transfer confirm, card-pay exclusion |
| W5 | LDG-2.4 (built; re-validate) · ENR-3.4 | **deduped true-spend** — the wedge endpoint |

> The **first-12-PR linear slice** (header) is W0+W1 flattened into a dependency-safe order: PLT-2.1 → PLT-2.3 → SEC-1.1 → SEC-1.3 → SEC-1.2 → LDG-4.3 → SRC-1.2 → CAT-1.1 → ENR-2.7 → ENR-2.1 → REV-1.1 → PLT-2.2. Perf-budget leaves (PLT-8.x, ING-6.x) slot in cross-phase and carry `_benchmark-by:_` (block A) rather than a wave. Gating spikes that must resolve before their dependents enter a wave: **AUT-5.1** (before household tables), **IOS-1.1** (before any IOS leaf), **SEC-5.2** (before CAT-4.1 LLM), **RCP-2.1** (before OCR), **SRC-1.5** (RESOLVED).

### F. Vision → leaf coverage matrix (v8 — the coverage proof)

> Closes iteration-07 self-critique #6: a terse map from every README §1 vision bullet + north-star question to the leaf IDs that satisfy it, regenerated from leaf bodies. Owner = the leaf group that carries the invariant; one representative leaf is **bolded**. Every bullet has ≥1 leaf — **no holes found**. (This is a coverage snapshot; the leaf remains canonical, per the consistency rule.)

| Vision bullet (§1) | Satisfying leaves (owner group bolded) |
|--------------------|----------------------------------------|
| True spend (exclude transfers/card-pay) | LDG-2.1/2.2/2.3/2.6, **LDG-2.4**, LDG-2.5, ENR-3.4 → RPT-2.4/5.3 |
| Reimbursements & receivables | **OWE-1.1**/1.2/1.3/1.4, OWE-2.1/2.2/2.3, ENR-4.3, REV-3.2 |
| Returns | **OWE-3.1**/3.2/3.3/3.4 (GF-RETURN), ENR-4.3 |
| Receipts & line items | RCP-1.x (capture), RCP-2.x (OCR/parse), **RCP-3.1**/3.2/3.3 (line items), RCP-4.x (invoices) |
| Cross-source enrichment & dedup | ENR-1.x (merchant), ENR-2.1/2.6/2.7 (match), **ENR-3.1**/3.2/3.3/3.4 (dedup), ING-4.2 |
| Smart LLM categorization that learns | CAT-4.1/4.4/4.5 (classify), CAT-4.2/4.3 (batch/threshold), **CAT-5.1**/5.2/5.3 (learning) |
| Prediction & anomaly | INT-1.x (recurring), INT-2.x (subs), INT-3.x (forecast), **INT-4.1**/4.2/4.3/4.4/4.5, INT-5.x (forgotten) |
| Fragmented money (bank/card/Venmo/PayPal/investments) | SRC-1.x (provider framework + SimpleFIN), **SRC-2.x** (bank/card), SRC-3 (Venmo), SRC-4 (PayPal), SRC-5 (investments), SRC-8 (manual/CSV/OFX) |
| Review queue (Tinder-style) | **REV-1.1**/1.2/1.3 (build), REV-2.x (swipe/keyboard), REV-3.x (card actions), REV-4.x |
| Web + iOS | NAV-1.x + all web epics (web); **IOS-1.x**/2.x/3.x/4.x (iOS, gated on IOS-1.1 spike) |
| Google/Clerk sign-in | AUT-1.1 (Clerk), **AUT-1.2** (Google), AUT-1.3 (prod instance), IOS-1.3 (iOS), SEC-1.1 (server-verify) |
| Push notifications | NTF-1.x (in-app), NTF-3.x (web push), **NTF-4.x** (iOS push), NTF-5.x (dispatch) |
| **NS: Where did money go?** | RPT-2.x (cash-flow/Sankey/click-filter), **RPT-3.x** (category breakdown/drilldown), CAT-1.x (taxonomy) |
| **NS: Where did it come from?** | LDG-1.4/2.1 (income/direction), RPT-2.1/2.2, **RPT-4.3** (income vs expense), RPT-5.3 |
| **NS: What is my true spending?** | **LDG-2.4** (+ GF-TRUESPEND), ENR-3.4, RPT-2.4/3.1/5.3/6.2 |
| **NS: What do I owe / am I owed?** | **OWE-1.x** (owed to me), OWE-3.x (returns), OWE-5.x (bills), LDG-3.4 (total owed) |
| **NS: What changed unexpectedly?** | **INT-4.x** (anomaly/overcharge/price-change), INT-5.2 (forgotten), INT-3.3 (forecast vs actual), RPT-4.1 (trends) |
| **NS: What is my financial state right now?** | **RPT-1.1** (summary cards), LDG-3.1/3.2/3.3 (balances/net worth), RPT-1.4/4.2, PLT-8.5 (dashboard latency) |
...
- **PLT-2.3 — Idempotent migration/backfill test harness** `[new]` `M` `data` — reusable fixture asserting re-run safety for data migrations.
  _deps:_ PLT-2.1 · _skill:_ db-evolution · _accept:_ harness runs any backfill twice and asserts no row delta on 2nd run. _test:_ pytest (`GF-REPLAY`: 2nd run row-count delta == 0 for every backfill that adopts the harness). _files:_ `apps/api/tests/conftest.py`, `apps/api/tests/helpers/backfill.py`. _xc:_ idem. **(v7: added the `PLT-2.1` edge — the migration/backfill replay harness presupposes the Alembic framework it exercises; this also makes the stated critical path `PLT-2.1 → PLT-2.3 → LDG-4.3` a real edge chain, not just a narrative ordering.)**
...
- **PLT-5.3 — Feature flag module + flag registry (server + client)** `[new]` `M` `shared` — flag eval shared by api + web; per-user overrides; **owns the flag-key registry** (the index-block "B. Feature-flag catalog" is its seeded source of truth, like ENR-2.7 owns match-config). **(v6: closes iteration-05 self-critique #6 — every flag key (`provider.simplefin`, `source.gmail`, `llm.categorize`, `household`, …) is enumerated + default-seeded here, so flag-gated leaves read a known key instead of inventing strings.)**
  _data:_ `feature_flags`(key unique, default_enabled bool, description) seeded idempotently from the catalog (index-block B) + optional per-user override row `feature_flag_overrides`(user_id, key, enabled) unique `(user_id, key)`. _accept:_ flag off hides feature in web and 404s its API; an unknown key fails closed (treated as off); re-seeding the catalog yields no dup rows. _test:_ vitest + pytest (every catalog key resolves; unknown key → off; per-user override beats default; `GF-REPLAY`: re-seed → 1 row/key). _files:_ `core/flags.py`, `apps/web/lib/flags.ts`, `models.py`, `apps/api/src/cash_lens_api/scripts/seed_flags.py`. _xc:_ authz, idem.
...
- **PLT-8.2 — Large-account backfill performance budget** `[new]` `M` `api` — bounded memory + batched writes for multi-year backfills.
  _deps:_ ING-1.1 · _accept:_ backfilling a 50k-txn account stays under a documented memory/time budget (batched, streamed). _test:_ pytest (synthetic 50k fixture asserts batch size + peak memory bound). _perf:_ batch size **500** rows/write; sustained ≥ **5,000 txns/min**; peak process RSS delta < **256 MB** for a 50k backfill (no full-list materialization). _benchmark-by:_ first PLT-8.4 run (P1, before ING-1.x backfill ships to prod) — throughput + RSS are HYP until measured against the 50k synthetic fixture; record the value and promote to SLO or re-set the batch size. _files:_ `services/sync.py`, `services/ingest.py`. _xc:_ obs:backfill.txns_per_min, rate.
...
- **PLT-8.5 — Dashboard summary latency budget + cache** `[new]` `M` `api` — keep the first-paint dashboard summary fast.
  _deps:_ RPT-1.1, PLT-9.2 · _data:_ cache `DashboardSummary` per user (PLT-9.2 key), invalidate on transaction write. _accept:_ `GET /dashboard` returns within budget at 100k events; cache hit avoids recompute. _test:_ pytest (latency on 100k seed; **stale-after-write**: a transaction write invalidates so the next `/dashboard` shows the new total, not a stale cached one). _perf:_ `GET /dashboard` p95 < **400ms** cold, < **80ms** warm at 100k events/user. _files:_ `routers/dashboard.py`, `services/dashboard.py`. _xc:_ authz, money, obs:http.server.duration.
- **PLT-8.4 — Load/scale test harness (seed N + smoke + burst mode)** `[new]` `M` `data` — reusable generator of N synthetic users/txns + perf smoke, **plus a webhook-burst/storm mode** so write-path back-pressure budgets are testable, not just asserted in prose. **(v7: closes iteration-06 open question #1 — ING-6.2's "200-item storm drains with peak concurrency ≤ cap" had no harness; the generator now fans in K simultaneous `SYNC_UPDATES_AVAILABLE`-style sync triggers and records peak in-flight concurrency, queue depth, and per-item exactly-once completion. This makes ING-6.2 (and ING-6.1's write-chain budget under load) the FIRST consumer of a burst run.)**
  _skill:_ testing-playbook · _data:_ generator covers ledger/raw/accounts/balances at scale (reused by 8.1–8.3 budgets) + a **burst harness** `make perf-burst K=200` that enqueues K sync jobs at once against the PLT-3.x task runner + PLT-9.1 shared store and captures `peak_in_flight`, `max_queue_depth`, deferral count, and per-item completion. _accept:_ `make perf-seed N=100000` populates realistic data and smoke asserts key endpoints under budget; **`make perf-burst K=200` drives ING-6.2's storm assertion — peak in-flight ≤ the configured cap, queue drains to zero, every item completes exactly once, no provider-quota breach.** _test:_ pytest + doc (seed smoke; **burst smoke fans in K triggers and asserts the recorded peak concurrency ≤ cap + zero dropped items, feeding ING-6.2/ING-6.1**). _perf:_ seed 100k txns in < 5 min on CI hardware; burst run of K=200 completes within the ING-6.2 drain budget. _benchmark-by:_ self — this leaf IS the load harness; its own seed-time + burst-drain numbers are measured the first time it runs in CI (P0/P1), establishing the baseline every other HYP budget is benchmarked against. _files:_ `apps/api/tests/perf/`, `apps/api/src/cash_lens_api/scripts/perf_seed.py`, `apps/api/src/cash_lens_api/scripts/perf_burst.py`. _xc:_ obs:backfill.txns_per_min,http.server.duration.
...
- **SEC-3.1 — Account deletion orchestration (export → disconnect → cascade)** `[new]` `M` `api` — orchestrate full erasure with confirmation + audit.
  _deps:_ SEC-3.5, SEC-3.2, SEC-4.1, SRC-2.5 · _data:_ deletes user + cascade across all 7 tables. _accept:_ post-deletion no rows for `user_id` anywhere and providers revoked. _test:_ pytest (delete then assert 0 rows across every table for that `user_id`; re-run delete is a no-op; a SEC-4.1 audit row is written before the cascade). _files:_ `services/account_deletion.py`, `routers/users.py`. _xc:_ authz, idem. _risk:_ **irreversible total erasure** — a bug or wrong `user_id` destroys all of a user's data with no undo; mitigate by completing the SEC-3.2 export FIRST (export→disconnect→cascade order), wrapping the cascade in one DB transaction (all-or-nothing), and writing a SEC-4.1 audit row before deleting — **SEC-3.2 + SEC-4.1 + SEC-3.5 are now hard `_deps:_` so this destructive leaf cannot ship before its safeguards exist** (v8 risk→dep promotion of SEC-4.1). The AUT-4.2 step-up re-auth stays advisory-not-a-dep: AUT-4.2 already deps SEC-3.1 (the step-up UI is built *for* this flow), so an SEC-3.1→AUT-4.2 edge would cycle — the ordering is enforced the other way (AUT-4.2 gates the *call site*).
- **SEC-3.5 — Cascade-delete migration + FK constraints** `[new]` `M` `data` — add ON DELETE cascade / explicit child deletes so erasure is total.
  _deps:_ PLT-2.1 · _skill:_ db-evolution · _data:_ FK `ondelete='CASCADE'` on child tables. _accept:_ deleting a user removes all dependent rows; migration is reversible. _test:_ pytest (orphan check). _files:_ `models.py`, `apps/api/alembic/versions/`. _xc:_ idem. _risk:_ **a mis-scoped CASCADE silently widens what a single DELETE destroys** (e.g. attaching it to the wrong FK could cascade-delete shared/household rows); mitigate by enumerating each child FK explicitly in the migration, asserting the downgrade restores `ON DELETE NO ACTION`, and gating the destructive SEC-3.1 path on this being verified — never rely on implicit cascade.
...
- **SEC-3.3 — Consent records per data source** `[new]` `M` `data` — store consent grant/scope/timestamp per connection.
  _data:_ `consents` table (user_id, source, scope, granted_at, revoked_at) unique `(user_id, source, scope)` so re-consent doesn't dup. _accept:_ connecting a source writes a consent row; disconnect sets revoked; re-connecting the same source/scope updates in place. _test:_ pytest (`GF-REPLAY`: re-consent same source/scope → 1 row). _files:_ `models.py`, `services/sources/__init__.py`. _xc:_ authz, idem.
...
- **AUT-3.3 — User settings/preferences store** `[new]` `M` `data` — generic key/value prefs (notif defaults, display options).
  _deps:_ PLT-2.1 · _data:_ `user_preferences` table (user_id, key, value) unique `(user_id, key)` or JSON column; upsert on write. _accept:_ prefs round-trip via API; re-saving the same key updates in place. _test:_ pytest (`GF-REPLAY`: re-save same key → 1 row, latest value). _files:_ `models.py`, `services/users.py`, `routers/users.py`. _xc:_ authz, idem.
...
- **ONB-4.1 — Demo workspace → real account migration** `[new]` `M` `api` — clear demo data when first real connection lands.
  _deps:_ PLT-1.2, SRC-2.2 · _data:_ delete demo-flagged rows; preserve user. _accept:_ first real connect removes demo transactions without touching real ones. _test:_ pytest (idempotent, demo-only deletion). _files:_ `services/demo_seed.py`, `services/plaid.py`. _xc:_ idem, authz. _risk:_ **a too-broad delete predicate could wipe a user's first REAL transactions along with demo data**; mitigate by scoping the delete strictly to rows carrying the explicit `is_demo`/demo-workspace flag (never "delete all of this user's rows"), running it only on the first real-connect transition, and a test asserting a pre-existing real row survives the demo purge.
...
- **SRC-1.7 — SimpleFIN adapter (deferred fallback provider, flag-gated)** `[new]` `M` `api` — implement the documented SRC-1.5 low-cost fallback as a real second `Provider` impl behind a feature flag, so the fallback is actionable code, not just a recommendation. **(v5: makes the SRC-1.5 decision's "ship behind the interface" concrete — mirrors SRC-1.6 for Plaid.)**
  _deps:_ SRC-1.1, SRC-1.2, PLT-5.3 · _flag:_ `provider.simplefin` (block B; default OFF — adapter + routes 404 when off) · _data:_ `services/sources/simplefin_adapter.py` conforming to the SRC-1.1 Protocol; connect = store the SimpleFIN **Access URL** (base64 setup-token → POST claim → access URL w/ Basic Auth) encrypted via SEC-1.3 on a `connections` row with `provider='simplefin'`, `provider_ref`=hash(access_url); sync = GET `/accounts` (balances + transactions JSON) → existing raw-upsert (ING-4.1); capabilities = `{transactions, balances}` only (NO investments, NO webhooks → poll-only). Behind `PLT-5.3` flag `provider.simplefin` (default OFF — see flag-catalog index-block B). _accept:_ with the flag on, a SimpleFIN access URL connects, syncs balances+transactions idempotently through the same `connections`/raw pipeline as Plaid, and its capability metadata correctly reports no-investments; flag off → the provider is unregistered and its routes 404; a bank also covered by Plaid produces distinct raw rows that the cross-source matcher then dedups (it is not double-counted). _test:_ pytest (contract suite vs the SRC-1.1 fake passes for the SimpleFIN adapter; mocked `/accounts` JSON → raw rows; `GF-REPLAY`: re-sync same `/accounts` payload → 0 new rows; capabilities exclude investments; flag-off → 404; `GF-DEDUP-XPROV`: a SimpleFIN row for a bank txn that Plaid also returns is a distinct raw row whose cross-provider dedup (asserted in ENR-3.1) counts the purchase once). _files:_ `apps/api/src/cash_lens_api/services/sources/simplefin_adapter.py`, `services/sources/__init__.py`, `core/crypto.py`, `models.py`. _xc:_ authz, crypto, idem, rate. _risk:_ SimpleFIN is consumer-subscription + poll-only (no webhooks/cursor) and has no investments API — keep DEFERRED behind the flag; only promote if an SRC-1.5 re-decide trigger fires.
...
- **SRC-1.3 — Provider capability metadata** `[new]` `S` `api` — declare per-provider fetchable data (txns, balances, holdings, identity).
  _deps:_ SRC-1.1 · _accept:_ UI can ask "does this provider support investments?" and get a truthful answer (served behind auth). _test:_ pytest. _files:_ `services/sources/__init__.py`, `schemas.py`, `packages/api-types/src/index.ts`. _xc:_ authz.
...
- **SRC-6.1 — Gmail OAuth connection** `[new]` `M` `api` — OAuth consent + read-only Gmail scope.
  _skill:_ env-vars · _deps:_ SEC-3.3 · _flag:_ `source.gmail` (block B; default OFF — blocked on CASA verification) · _accept:_ user grants Gmail read scope and a connection is created. _test:_ pytest (mocked OAuth) + manual. _files:_ `services/sources/gmail.py`, `routers/connections.py`, `core/crypto.py`. _xc:_ authz, crypto, rate. _risk:_ `gmail.readonly` is a Google **restricted** scope → CASA tier-2 security assessment + verification (weeks–months, recurring annual cost ~$mid-hundreds–low-thousands) before prod; gate behind the `source.gmail` flag and ship the SRC-7.2 forward-to-email fallback first.
...
- **ING-6.1 — Ingest→normalize→match→dedup write-chain budget** `[new]` `M` `api` — bound the per-batch latency + throughput of the full write pipeline that runs on every sync (raw upsert → ledger normalize → candidate-gen/score → dedup grouping), since matching/dedup add write-amplification on top of plain ingest. **(v5: closes iteration-04 self-critique #4 — the write chain had no p95/throughput budget. Read perf lives in PLT-8; this is its write-path counterpart.)**
  _deps:_ ING-4.1, LDG-1.1, ENR-2.6, ENR-3.1, PLT-8.4 · _data:_ no schema change; instruments the existing chain; emits the contract histogram `sync.duration` + counter `match.links{band}`; bounded match candidate fan-out per new event (blocking key from ENR-2.7 caps comparisons). _accept:_ ingesting a 1,000-new-txn sync batch completes the full normalize+match+dedup chain within budget without unbounded match comparisons (blocking keeps it ~linear, not O(n²)). _test:_ pytest (synthetic 1k-new-txn batch asserts end-to-end p95 + that per-event candidate comparisons stay bounded by the blocking key, never a full cross-join; reuses the PLT-8.4 generator). _perf:_ full write chain sustained ≥ **2,000 txns/min** end-to-end (normalize+match+dedup, not raw-only); per-100-txn batch p95 < **800ms**; match candidate comparisons per new event bounded (no O(n²) cross-join). _benchmark-by:_ first PLT-8.4 run at the P3→P4 load gate (the write chain only exists once ENR-2.6/3.1 land) — the throughput + p95 are HYP reasoned from the read tier; measure on the 1k-new-txn batch and promote to SLO or re-tune the batch size + blocking key. _files:_ `services/ingest.py`, `services/ledger/normalize.py`, `services/enrichment/match.py`, `services/enrichment/dedup.py`. _xc:_ money, idem, obs:sync.duration,match.links.
- **ING-6.2 — Sync-burst concurrency + back-pressure budget** `[new]` `M` `api` — bound the COMBINED load when many items' webhooks fire at once (a webhook storm): cap concurrent sync jobs per Cloud Run instance, set a queue-depth ceiling, and shed/defer rather than overrun the ING-5.2 rate limiter or the ING-6.1 match write-amplification. **(v6: closes iteration-05 open question #2 — ING-6.1 budgets one batch, but the *aggregate* burst (Plaid `SYNC_UPDATES_AVAILABLE` for N items x Cloud Run instances x shared rate limiter x match write-amp) was unmodeled. This leaf is the system-level back-pressure budget that makes the per-batch budget hold under burst.)**
  _deps:_ ING-3.2, ING-5.2, ING-6.1, PLT-3.2, PLT-9.1, PLT-8.4 · _data:_ no schema change; sets Cloud Tasks dispatch caps (`maxConcurrentDispatches`, `maxDispatchesPerSecond`) + Cloud Run container concurrency, and a per-instance in-flight semaphore + a shared queue-depth gauge in PLT-9.1; on breach, new webhook-triggered syncs are **deferred** (re-enqueued with backoff via PLT-3.5), never dropped or run unbounded; emits `jobs.enqueued{outcome=deferred}`. _accept:_ a simulated storm of 200 simultaneous `SYNC_UPDATES_AVAILABLE` webhooks (driven by the PLT-8.4 `perf-burst` harness) drains within budget with bounded peak concurrency — no provider 429 cascade, no match O(n^2) blowup, no dropped item (every item eventually syncs exactly once). _test:_ pytest (uses the **PLT-8.4 burst harness** to fan in 200 sync triggers at once → assert recorded peak in-flight ≤ the cap, queue drains to zero, each item synced exactly once via `GF-REPLAY`, deferrals re-run; no provider-quota breach using the ING-5.2 shared bucket). _perf:_ ≤ **8** concurrent sync jobs per instance (tunable); queue-depth alert threshold **500**; storm of 200 items drains with peak concurrency ≤ cap and zero dropped items; combined write throughput stays within the ING-6.1 per-batch budget. _benchmark-by:_ first PLT-8.4 **burst-mode** run at the P3→P4 load gate — the cap (8), queue ceiling (500), and drain time are HYP until that run measures real peak concurrency + drain under storm; promote to SLO or re-tune from the measured values, never ship the guess as a contract. _files:_ `services/sync.py`, `core/tasks.py`, `core/rate_limit.py`, `routers/plaid.py`. _xc:_ rate, idem, obs:jobs.enqueued. _risk:_ under-provisioned caps cause storm backlog (slow but safe); over-provisioned caps trip provider 429s + match write-amp — the budget numbers are HYP (see perf index) to validate in the first load test (PLT-8.4 burst mode), then promote to SLO.
...
- **LDG-2.4 — True-spend computation** `[built]` `M` `api` — spend excluding transfers/card payments/excluded.
  _deps:_ LDG-2.3, LDG-2.6 · _data:_ feeds `DashboardSummary.true_spend_this_month`. _accept:_ true-spend = Σ(outflow) − transfers − card-payments − excluded − refunds (− settled receivables, net of returns); deduped purchases count their canonical member once. _test:_ pytest (`GF-TRUESPEND`: exact total on the fixed mixed-month dataset; `GF-COMBO-SRR`: the $120 split-with-2-reimbursable-and-1-return parent contributes only the payer's net $30 while the return is open and $0 once it refunds — reimbursed shares and the refunded return are each removed exactly once, not double-subtracted; `GF-COMBO-STACK`: the deduped $96 dinner split 3×$32 with 2 shares reimbursed contributes the payer's net **$32 once**, never $288 (3 sources × 3 splits) — dedup, split, and reimbursement each apply exactly once). _files:_ `services/dashboard.py`, `services/ledger/true_spend.py`. _xc:_ money.
- **LDG-2.5 — Manual exclude-from-spend override** `[built]` `S` `web` — user toggles `exclude_from_spend`.
  _data:_ `PATCH /transactions/{id}` `exclude_from_spend`. _accept:_ toggling immediately changes true-spend total. _test:_ playwright + pytest (toggle PATCH p95 incl. cache invalidation asserted). _perf:_ toggle `PATCH /transactions/{id}` p95 < **200ms** at 100k events/user — the write only flips the flag + bumps the PLT-9.2 cache version; the true-spend total is recomputed lazily on the next read, not synchronously on this write. _benchmark-by:_ first PLT-8.4 run at the P3→P4 load gate — the write-path p95 is HYP until measured on a 100k seed; promote to SLO or re-examine if the cache-version bump is on the hot path. _files:_ `routers/transactions.py`, `apps/web/components/transaction-editor.tsx`. _xc:_ money, authz, a11y.
...
- **LDG-4.4 — Cut over reads/writes to minor units** `[new]` `M` `api` — switch app + API + shared types to integer money; drop float reliance.
  _deps:_ LDG-4.3 · _data:_ `amount`/balances serialized as minor-unit ints in `schemas.py` + `api-types`. _accept:_ no money arithmetic uses Float; serialized amounts are ints. _test:_ pytest + vitest. _files:_ `schemas.py`, `services/*.py`, `packages/api-types/src/index.ts`, `apps/web/lib/money.ts`. _xc:_ money. _risk:_ **a partial cutover (some reads minor-unit, some still float) double-scales or mis-displays money** (e.g. a $19.99 row shown as $1,999); mitigate by gating on the LDG-4.3 backfill+reconcile passing first, flipping the API serializer + the web `money.ts` formatter in ONE change (shared `api-types` forces both ends together), and a contract test asserting the wire amount is an int end-to-end before any float read path is removed.
- **LDG-4.1 — Multi-currency amount model** `[new]` `M` `data` — store amount + currency consistently.
  _deps:_ LDG-4.4 · _data:_ `(amount_minor, currency)` pairs; `iso_currency_code` already present. _accept:_ a EUR transaction stores EUR, not silently USD; re-normalizing an event preserves its stored currency (no double-conversion). _test:_ pytest (`GF-FX`+`GF-REPLAY`: re-deriving a EUR event keeps `currency=EUR`, amount unchanged; `GF-COMBO-FX`: the £90 dinner's canonical member keeps 9000 GBP-minor through dedup + split + reimburse — no stage rewrites the stored currency to USD). _files:_ `models.py`, `services/fx.py`. _xc:_ money, idem.
- **LDG-4.2 — FX rate fetch + conversion** `[new]` `M` `api` — fetch daily FX; convert to primary currency for rollups.
  _deps:_ LDG-4.1 · _data:_ `fx_rates`(base, quote, date, rate) unique `(base, quote, date)`. _accept:_ multi-currency totals convert to primary currency at txn-date rate; missing direct rate triangulates via an intermediate; missing-rate day falls back to last-known, never silent 1:1. _test:_ pytest (`GF-FX`: €100 @1.10 → $110 in USD rollup; a day with no rate uses prior rate and is flagged, not parity; `GF-FX-TRI`: £50 with only EUR↔USD + GBP↔EUR converts GBP→EUR→USD; no path → flagged unconvertible, not zeroed; `GF-COMBO-FX`: the £90 dinner triangulates GBP→EUR→USD ≈ $115.83 as a *report-time* rollup while the stored amount stays 9000 GBP-minor — conversion is non-destructive, and converting-then-summing vs summing-then-converting agree within ±1 minor unit). _files:_ `services/fx.py`, `models.py`. _xc:_ money, rate. _risk:_ FX source reliability; cache + fallback to last-known rate (explicit, flagged — never assume 1:1).
...
- **ENR-2.1 — Match candidate generation (blocking by amount+date)** `[new]` `M` `data` — generate candidate pairs across sources via blocking keys.
  _deps:_ ENR-2.7, LDG-4.3 · _data:_ candidate index on `(user_id, amount_minor, date)`; ±window + exact-`amount_minor` blocking key both from ENR-2.7's `match_config`. _accept:_ given two sources of one purchase, the pair is a candidate; a same-txn duplicate arriving from two different providers is also a candidate (different `provider`, same amount/date). _test:_ pytest (`GF-DEDUP`: bank+Venmo+receipt of the $42.17 purchase generate the candidate pairs; unrelated same-amount rows **outside the ±3d window** don't; `GF-DEDUP-XPROV`: a Plaid row + a SimpleFIN row for the same −$73.40 bank txn generate a candidate pair — cross-provider blocking works because they are distinct raw rows, not collapsed by ING-4.1 provider-id idempotency). _files:_ `services/enrichment/match.py`, `services/enrichment/match_config.py`, `models.py`. _xc:_ money, idem.
...
- **ENR-3.1 — Cross-source dedup model (canonical purchase + members)** `[new]` `M` `data` — one purchase = one canonical record with N source members.
  _skill:_ db-evolution · _deps:_ ENR-2.7, ENR-2.6 · _data:_ `purchase_groups`(id, user_id, canonical_event_id) + `purchase_group_members`(group_id, event_id) unique `event_id` (each ledger event in ≤1 group); only `match_links` at/above the auto-link band (from ENR-2.7) or user-confirmed form groups; canonical chosen by source precedence **bank > card > Venmo/PayPal > receipt**, with a provider tie-break **Plaid > SimpleFIN** when two members are the same source-type from different providers (so a primary-provider duplicate wins, per `GF-DEDUP-XPROV`). _accept:_ bank+Venmo+receipt of one purchase roll up to a single canonical amount counted once; two charges from the SAME source never group (dedup requires distinct sources); the same bank txn arriving from two providers groups to one canonical (Plaid wins the tie). _test:_ pytest (`GF-DEDUP`: 3 sources → exactly 1 purchase_group, canonical = bank row; a 0.7 review-band link does NOT auto-group; `GF-COMBO-DUPXFER`: two same-account same-source −$60 charges do NOT form a purchase_group — they are a real double-charge, not one purchase seen twice; `GF-DEDUP-XPROV`: a Plaid row + a SimpleFIN row for the same bank txn group to 1 canonical, Plaid chosen by tie-break; `GF-COMBO-STACK`: 3 sources of the $96 dinner form exactly 1 group whose canonical member is the one the later split attaches to — the 3 members do not each spawn a split; `GF-COMBO-FX`: the 3 sources of the £90 dinner form 1 group whose canonical keeps GBP-minor — the FX rollup happens at report time, never as a per-member rewrite, so dedup is currency-agnostic). _files:_ `services/enrichment/dedup.py`, `models.py`. _xc:_ idem, money. _risk:_ **a wrong auto-merge collapses two genuinely-distinct purchases into one, hiding real spend** (and a wrong canonical pick mis-attributes amount/category); mitigate by grouping ONLY at/above the auto-link band (review-band links go to a human via ENR-3.2, never auto-group), keeping every source member intact + linked (the merge is a grouping, NOT a row deletion — non-destructive and reversible via ENR-3.3), and the `GF-COMBO-DUPXFER` negative control asserting same-source charges never group.
...
- **ENR-3.3 — Unmerge / split-back action** `[new]` `S` `api` — reverse a merge.
  _deps:_ ENR-3.1 · _accept:_ unmerge restores independent events and totals. _test:_ pytest (unmerge a 3-member group → 3 independent events, each re-counted once; downstream splits/receivables on the canonical are detached, not orphaned; re-merge then unmerge is stable). _files:_ `services/enrichment/dedup.py`, `routers/transactions.py`. _xc:_ idem, authz. _risk:_ **unmerging a group that already has splits/receivables/line-items attached to its canonical member can orphan or double-count those children**; mitigate by blocking unmerge (or cascading a clean detach with explicit user confirm) when the canonical has dependent splits/receivables, and asserting post-unmerge that no child points at a now-dissolved canonical.
...
- **ENR-4.1 — Split transaction model (parent + child splits)** `[new]` `M` `data` — model a transaction split into N parts summing to the whole.
  _deps:_ LDG-4.3 · _data:_ `transaction_splits`(id, parent_event_id, amount_minor, category_id) index `(parent_event_id)`; parent flagged `is_split`; when the parent is a deduped purchase, `parent_event_id` is the `purchase_groups.canonical_event_id` (splits attach to the canonical member, never a non-canonical source row). _accept:_ Σ(child amount_minor) == parent amount_minor exactly; reject otherwise; a split on a deduped parent sums against the canonical amount, not the member count. _test:_ pytest (`GF-SPLIT`: $90 → 3×$30 saves; $90 → $30+$30+$25 rejected; integer-exact, no float drift; `GF-COMBO-STACK`: the $96 dinner deduped from 3 sources splits 3×$32 against the single canonical $96 — Σ=$96 once, NOT 3×$96, and the split attaches to the canonical event id only; `GF-COMBO-FX`: the £90 dinner's split sums to **9000 GBP-minor exactly in the native currency** (NOT to the rounded converted USD — splitting $115.83 three ways would lose a cent), and each share converts for display only). _files:_ `models.py`, `services/enrichment/splits.py`. _xc:_ money, idem.
...
- **ENR-4.3 — Split for reimbursement (mark portion owed)** `[new]` `M` `api` — mark a split as receivable.
  _deps:_ ENR-4.1, OWE-1.1 · _accept:_ marking a split reimbursable creates a receivable for that amount; a single parent can carry split + reimbursable + return states at once without cross-wiring; reimbursing splits of a deduped parent does not fan out across the parent's source members. _test:_ pytest (`GF-COMBO-SRR`: on a $120 parent split 4×$30, marking 2 shares reimbursable creates exactly 2 receivables and leaves the 1 return-flagged share independent — settling a receivable does not touch the return, and the return's refund does not touch a receivable; `GF-COMBO-STACK`: on the deduped $96 dinner split 3×$32, marking 2 shares reimbursable creates exactly **2** receivables of $32, NOT 6 — the 3 dedup members must not each spawn receivables; `GF-COMBO-FX`: on the £90 dinner split 3 ways, marking 2 shares reimbursable creates exactly **2** GBP-denominated receivables (matched/settled by GBP-equivalent inflows in native amount), NOT 6 and NOT silently USD). _files:_ `services/enrichment/splits.py`, `services/receivables.py`. _xc:_ money, idem, authz.
...
- **RCP-3.2 — Line items ↔ ledger linkage** `[new]` `M` `data` — connect receipt lines to the matched transaction/splits.
  _deps:_ ENR-4.1, ENR-2.4 · _accept:_ a matched receipt's lines attach to the transaction (the canonical member if the txn is deduped) and can drive splits; lines never attach to a non-canonical source row of a purchase group. _test:_ pytest (`GF-COMBO-STACK`: the receipt that is one of the 3 dedup sources of the $96 dinner attaches its line items to the canonical event, and those lines can seed the 3×$32 split — line totals reconcile to the canonical $96, not to a duplicated member). _files:_ `services/receipts.py`, `services/enrichment/splits.py`, `models.py`. _xc:_ money, idem.
...
- **CAT-1.4 — Category merge / rename / archive** `[new]` `M` `api` — lifecycle ops that reassign affected events.
  _deps:_ CAT-1.3 · _accept:_ merging A into B reassigns A's events to B; archive hides without data loss. _test:_ pytest (reassignment + idempotent). _files:_ `routers/categories.py`, `services/categorize.py`. _xc:_ idem, authz. _risk:_ **merge bulk-reassigns every event of category A and then drops A — irreversible if mis-targeted** (and it silently rewrites historical reports); mitigate by making merge a single transactional reassign-then-archive (archive A, don't hard-delete, so it's recoverable), recording the reassignment in CAT-5.1-style history, requiring explicit confirm of the target, and asserting the reassign count equals A's prior event count (no rows stranded).

### CAT-2 Manual categorization
- **CAT-2.1 — Inline category edit on transaction** `[built]` `S` `web` — edit category in the transaction list.
  _data:_ `PATCH /transactions/{id}` `category/subcategory`. _accept:_ edit persists and updates reports. _test:_ playwright + pytest (server PATCH p95 asserted on 100k-event seed). _perf:_ single `PATCH /transactions/{id}` p95 < **200ms** at 100k events/user incl. the PLT-9.2 cache invalidation it triggers (no full report recompute on the write path). _benchmark-by:_ first PLT-8.4 run at the P3→P4 load gate — HYP until measured on a 100k seed; promote to SLO or re-set once the real PATCH+invalidate cost is known. _files:_ `routers/transactions.py`, `services/transactions.py`, `apps/web/components/transaction-editor.tsx`. _xc:_ authz, a11y.
- **CAT-2.2 — Bulk categorize selection** `[new]` `M` `web` — multi-select → set category.
  _deps:_ CAT-2.3 · _data:_ batch `PATCH /transactions` (id list + category) in one transaction; single cache-invalidation after the batch. _accept:_ selecting N txns and categorizing updates all N. _test:_ playwright + pytest (batch of 500 updates all 500 in one DB txn; cache invalidated once, not per-row). _perf:_ bulk PATCH of **500** ids p95 < **1s** (single multi-row UPDATE + one PLT-9.2 invalidation, never N round-trips). _benchmark-by:_ first PLT-8.4 run at the P3→P4 load gate — HYP until the 500-id multi-row UPDATE is measured on a 100k seed; promote to SLO or re-set the batch ceiling. _files:_ `routers/transactions.py`, `services/transactions.py`. _xc:_ authz, a11y, idem.
...
- **CAT-4.2 — Batch categorize uncategorized** `[new]` `M` `api` — async batch over uncategorized events.
  _deps:_ CAT-4.1, CAT-4.5, PLT-3.1 · _accept:_ batch job categorizes the backlog and is resumable. _test:_ pytest (resumes from checkpoint; `GF-REPLAY`: re-run categorizes only still-uncategorized rows). _perf:_ sustained ≥ **300 events/min** throughput with the CAT-4.5 cache warm; bounded concurrency so LLM rate-limit/budget never tripped; checkpoint every **200** events. _benchmark-by:_ first batch-categorize run against the REAL chosen LLM (P2/P4, after SEC-5.2 picks the provider/tier) — throughput is HYP and LLM-latency dependent; measure end-to-end with the cache warm and promote to SLO or re-set the concurrency cap to stay under the provider rate limit. _files:_ `services/categorize.py`. _xc:_ idem, rate, obs:llm.calls,llm.tokens,jobs.processed.
...
- **OWE-3.2 — Mark purchase "to return"** `[new]` `S` `web` — flag a purchase for return.
  _deps:_ OWE-3.1 · _accept:_ marking shows a "to return" badge + expected refund. _test:_ playwright. _files:_ `apps/web/components/transaction-editor.tsx`, `apps/web/components/status-badge.tsx`. _xc:_ money, a11y.
...
- **INT-3.4 — Cash-flow forecast horizons (30/60/90d) API** `[new]` `M` `api` — expose forecast with scheduled + recurring + trend.
  _deps:_ INT-3.1, OWE-4.1 · _data:_ `GET /forecast?horizon=` + shared `Forecast` type (minor-unit balances). _accept:_ returns projected balance at 30/60/90d, scoped to the authenticated user. _test:_ pytest (`GF-FORECAST`: horizon=30 → projected balance $4,285 exactly; deterministic; cross-user request → 404). _files:_ `routers/forecast.py`, `services/forecast.py`, `packages/api-types/src/index.ts`. _xc:_ authz, money.
...
- **RPT-3.2 — Category drilldown (e.g., groceries detail)** `[new]` `M` `web` — drill into a category's merchants/txns.
  _deps:_ RPT-3.1 · _accept:_ drilldown lists the category's transactions + merchant split (minor-unit amounts, deduped/true-spend aware). _test:_ playwright. _files:_ `apps/web/app/(app)/reports/`, `apps/web/components/charts/category-donut.tsx`. _xc:_ money, a11y.
- **RPT-3.3 — Health spend breakdown (derm/psych/glasses)** `[new]` `S` `web` — themed subcategory view.
  _deps:_ RPT-3.2 · _accept:_ health subcategories roll up correctly (minor-unit totals). _test:_ vitest. _files:_ `apps/web/app/(app)/reports/`, `apps/web/components/charts/category-donut.tsx`. _xc:_ money, a11y.
...
- **NTF-3.1 — Service worker + push subscription (VAPID)** `[new]` `M` `web` — register SW + subscribe with VAPID.
  _flag:_ `notif.web_push` (block B; default OFF — gates VAPID registration + send) · _accept:_ granting permission stores a push subscription server-side. _test:_ manual + vitest (subscribe logic). _files:_ `apps/web/public/sw.js`, `apps/web/lib/push.ts`. _xc:_ authz, crypto, a11y.
...
- **NTF-3.2 — Web push send pipeline** `[new]` `M` `api` — send web push via the dispatch service.
  _deps:_ PLT-3.1, NTF-3.3 · _accept:_ a triggered event delivers a web push to subscribed devices; send rate respects the push-service limits (batched/throttled, not unbounded fan-out). _test:_ pytest (mocked push). _files:_ `services/push.py`, `services/notifications.py`. _xc:_ rate, obs:notif.sent, idem.

### NTF-4 iOS push
- **NTF-4.1 — iOS push (APNs) registration** `[new]` `M` `ios` — register device token.
  _deps:_ IOS-1.2 · _flag:_ `notif.ios_push` (block B; default OFF — gates APNs push) · _accept:_ app registers and stores an APNs token. _test:_ manual. _files:_ `apps/ios/src/push/`, `routers/notifications.py`. _xc:_ authz.
- **NTF-4.2 — iOS push send pipeline** `[new]` `M` `api` — send via APNs.
  _deps:_ PLT-3.1, NTF-4.1 · _accept:_ a triggered event delivers an APNs push; send rate respects APNs throttling (batched, not unbounded fan-out). _test:_ pytest (mocked APNs). _files:_ `services/push.py`, `services/notifications.py`. _xc:_ rate, obs:notif.sent, idem.
...
_End of v8 — the FINAL consolidation pass. Index blocks A–F were regenerated from the leaves and diffed (leaf-as-truth): block A (perf) and E (build-order/waves/critical-path) verified consistent; block B (flags) reconciled by adding a `_flag:_` key tag to 9 owning leaves; block C (fixtures) primary-leaf lists extended for GF-DEDUP/GF-TRUESPEND; block F (vision→leaf coverage matrix) added — every §1 vision bullet + north-star question maps to ≥1 leaf, **no holes**. One risk→dep promotion landed (`SEC-3.1→SEC-4.1`, audit-before-destruction), keeping the graph acyclic (346 nodes, 361 edges). The header "auth chain" narrative was corrected to graph-truth. **346 leaves, all v0–v7 IDs preserved, IDs stable. This backlog is dispatch-ready: hand W0 (index-block E) to agents and follow `_deps:_`.**_
