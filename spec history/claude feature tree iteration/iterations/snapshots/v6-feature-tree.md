...
Version: **v6** · See [00-README.md](00-README.md) for vision, conventions, legend, and the iteration protocol.
...
- `_deps:_` upstream leaf IDs that must land first. **(v2: pruned to direct/non-transitive edges; graph verified acyclic — 0 cycles, 0 dangling. v3: re-verified after PLT-9.1/PLT-4.6 edges. v4: re-verified after ENR-2.7 + SRC-1.5. v5: re-verified after new SRC-1.7 SimpleFIN adapter + ING-6.1 write-path-budget edges. v6: re-verified after new ING-6.2 burst/back-pressure budget edges — 347 nodes, acyclic.)**
...
## Index blocks (v6 navigability — reference tables, NOT leaves)

> These four blocks (A perf budgets · B feature flags · C golden fixtures · D file-path convention) collect numbers/keys/fixtures that were previously scattered across leaves, so a reviewer can scan them in one place. They restate, never replace, the owning leaf — the leaf remains canonical.

### A. Performance-budget index (every `_perf:_` target in one table)

Each number is tagged **SLO** (a contract the implementer must hold — derived from a read-tier budget that has an EXPLAIN/index basis) or **HYP** (*hypothesis-to-benchmark* — a reasoned target to validate in the first PLT-8.4 perf run, NOT yet a contract; iteration-05 self-critique #2). Tier: R=read, W=write.

| Leaf | Tier | Budget | Class |
|------|------|--------|-------|
| PLT-8.1 | R | txn page (100 rows) p95 < 150ms @100k; index-only keyset | **SLO** |
| PLT-8.2 | W | backfill ≥ 5,000 txns/min; batch 500; RSS Δ < 256 MB @50k | **HYP** (throughput/memory unmeasured) |
...
| PLT-8.4 | — | seed 100k txns < 5 min on CI | **HYP** (CI-hardware dependent) |
| ING-6.1 | W | write chain ≥ 2,000 txns/min e2e; per-100 batch p95 < 800ms; bounded fan-out | **HYP** (reasoned from read tier, unmeasured) |
| ING-6.2 | W | ≤ 8 in-flight sync jobs/instance; queue depth alert > 500; shed/defer on breach | **HYP** (burst load unmodeled — validate in load test) |
| LDG-2.5 | W | exclude-toggle PATCH p95 < 200ms @100k (flag flip + cache-version bump) | **HYP** |
| CAT-2.1 | W | single category PATCH p95 < 200ms @100k (+ cache invalidation) | **HYP** |
| CAT-2.2 | W | bulk PATCH 500 ids p95 < 1s (one multi-row UPDATE + one invalidation) | **HYP** |
| CAT-4.2 | W | batch-categorize ≥ 300 events/min warm; checkpoint every 200 | **HYP** (LLM-latency dependent) |
...
- **ING-6.2 — Sync-burst concurrency + back-pressure budget** `[new]` `M` `api` — bound the COMBINED load when many items' webhooks fire at once (a webhook storm): cap concurrent sync jobs per Cloud Run instance, set a queue-depth ceiling, and shed/defer rather than overrun the ING-5.2 rate limiter or the ING-6.1 match write-amplification. **(v6: closes iteration-05 open question #2 — ING-6.1 budgets one batch, but the *aggregate* burst (Plaid `SYNC_UPDATES_AVAILABLE` for N items x Cloud Run instances x shared rate limiter x match write-amp) was unmodeled. This leaf is the system-level back-pressure budget that makes the per-batch budget hold under burst.)**
  _deps:_ ING-3.2, ING-5.2, ING-6.1, PLT-3.2, PLT-9.1 · _data:_ no schema change; sets Cloud Tasks dispatch caps (`maxConcurrentDispatches`, `maxDispatchesPerSecond`) + Cloud Run container concurrency, and a per-instance in-flight semaphore + a shared queue-depth gauge in PLT-9.1; on breach, new webhook-triggered syncs are **deferred** (re-enqueued with backoff via PLT-3.5), never dropped or run unbounded; emits `jobs.enqueued{outcome=deferred}`. _accept:_ a simulated storm of 200 simultaneous `SYNC_UPDATES_AVAILABLE` webhooks drains within budget with bounded peak concurrency — no provider 429 cascade, no match O(n^2) blowup, no dropped item (every item eventually syncs exactly once). _test:_ pytest (fan in 200 webhook events at once → assert peak in-flight ≤ the cap, queue drains, each item synced exactly once via `GF-REPLAY`, deferrals re-run; no provider-quota breach using the ING-5.2 shared bucket). _perf:_ ≤ **8** concurrent sync jobs per instance (tunable); queue-depth alert threshold **500**; storm of 200 items drains with peak concurrency ≤ cap and zero dropped items; combined write throughput stays within the ING-6.1 per-batch budget. _files:_ `services/sync.py`, `core/tasks.py`, `core/rate_limit.py`, `routers/plaid.py`. _xc:_ rate, idem, obs:jobs.enqueued. _risk:_ under-provisioned caps cause storm backlog (slow but safe); over-provisioned caps trip provider 429s + match write-amp — the budget numbers are HYP (see perf index) to validate in the first load test (PLT-8.4 generator extended to burst mode), then promote to SLO.
...
- **ENR-4.1 — Split transaction model (parent + child splits)** `[new]` `M` `data` — model a transaction split into N parts summing to the whole.
  _deps:_ LDG-4.3 · _data:_ `transaction_splits`(id, parent_event_id, amount_minor, category_id) index `(parent_event_id)`; parent flagged `is_split`; when the parent is a deduped purchase, `parent_event_id` is the `purchase_groups.canonical_event_id` (splits attach to the canonical member, never a non-canonical source row). _accept:_ Σ(child amount_minor) == parent amount_minor exactly; reject otherwise; a split on a deduped parent sums against the canonical amount, not the member count. _test:_ pytest (`GF-SPLIT`: $90 → 3×$30 saves; $90 → $30+$30+$25 rejected; integer-exact, no float drift; `GF-COMBO-STACK`: the $96 dinner deduped from 3 sources splits 3×$32 against the single canonical $96 — Σ=$96 once, NOT 3×$96, and the split attaches to the canonical event id only). _files:_ `models.py`, `services/enrichment/splits.py`. _xc:_ money, idem.
...
_End of v6. Subsequent passes (v7–v8) continue refining the whole tree per the quality bar in 00-README.md §4. Leaf IDs are stable; splits keep the original ID and add new sibling numbers._
