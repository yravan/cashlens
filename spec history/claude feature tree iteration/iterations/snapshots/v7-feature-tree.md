...
Version: **v7** · See [00-README.md](00-README.md) for vision, conventions, legend, and the iteration protocol.
...
> - `GF-COMBO-STACK` (v6) — **stacked three-branch fixture: a deduped multi-source parent that is THEN split** (closes iteration-05 self-critique #1 — prior combos were pairwise; this one stacks dedup → split → reimbursable on one chain). A $96 dinner is seen via **bank + Venmo + email receipt** (3 sources of one purchase, `GF-DEDUP`) → collapses to **one** canonical $96 purchase (bank chosen by precedence). That single canonical purchase is then **split 3 ways** ($32 each, `GF-SPLIT`), and **2 of the 3 shares are marked reimbursable** (`GF-COMBO-SRR`-style). Invariants that must ALL hold in order: (a) the 3 source rows form exactly **1** `purchase_group` and the split attaches to the **canonical** member, never to a non-canonical source row (a split on the deduped parent must not multiply when the parent has 3 members); (b) Σ(3 splits)=$96 exactly against the *canonical* amount, not 3×$96; (c) marking 2 shares reimbursable creates exactly **2** receivables of $32 (not 6 — the dedup members must not each spawn receivables); (d) true-spend counts the payer's net $32 once (one purchase, deduped, minus 2 reimbursed shares), never $288 (3 sources × 3 splits); (e) `GF-REPLAY`: re-deriving preserves the single group + its splits + receivables with zero row delta. Proves dedup-then-split-then-reimburse compose without member-fan-out double counting. (Wired: ENR-3.1, ENR-4.1, ENR-4.3, RCP-3.2, LDG-2.4.)
> - `GF-DEDUP-XPROV` (v6) — **cross-PROVIDER dedup variant of `GF-DEDUP`** (closes iteration-05 self-critique #3 — when both Plaid AND the SRC-1.7 SimpleFIN adapter cover overlapping institutions, the SAME bank transaction arrives once from each provider). A single −$73.40 grocery charge is ingested as a `provider='plaid'` raw row AND a `provider='simplefin'` raw row for the same underlying institution+account+date+amount. Invariants: (a) the two rows are DISTINCT raw rows (different `provider`/`provider_ref`, so ING-4.1's provider-txn-id idempotency does NOT collapse them — they are not the same source row); (b) cross-source matching (ENR-2.1/2.6) generates the candidate pair and scores ≥ auto-link band (exact amount + same date + same merchant), so dedup (ENR-3.1) groups them into **1** canonical purchase counted **once** in true-spend (`GF-DEDUP` generalizes across providers, not just bank-vs-Venmo); (c) canonical precedence resolves provider ties deterministically (Plaid > SimpleFIN, an explicit extension of the bank>card>Venmo>receipt order so a primary-provider duplicate wins); (d) `GF-COMBO-DUPXFER` stays distinct — two SimpleFIN-only same-account same-direction charges are still a double-charge, not a cross-provider dup, because cross-provider dedup requires *different* providers. Proves the multi-provider path (Plaid + SimpleFIN over the same bank) doesn't double-count. (Wired: ENR-2.1, ENR-3.1, SRC-1.7.)
> - `GF-COMBO-FX` (v7) — **the hardest unmodeled collision: a triangulated FX conversion INSIDE a split INSIDE a dedup group** (closes iteration-06 self-critique #2 — prior combos stacked dedup/split/reimburse but never a multi-currency leg, where rounding can drift across the conversion *and* the split simultaneously). Primary currency USD. A **£90 London dinner** is seen via **bank + the card's foreign-txn row + an email receipt** (3 sources of one purchase → `GF-DEDUP`/`GF-DEDUP-XPROV`-style), and on the conversion date only **EUR↔USD (1.10)** and **GBP↔EUR (1.17)** rates exist (no direct GBP→USD, so `GF-FX-TRI` triangulation applies: £90 → €105.30 → $115.83). That single canonical purchase is then **split 3 ways**, and **2 shares are marked reimbursable** (`GF-COMBO-STACK`-style). Invariants that must ALL hold in order: (a) the 3 sources form exactly **1** `purchase_group`; the stored amount stays **9000 GBP-minor** on the canonical member — conversion is a *report-time rollup*, never a destructive rewrite of the stored currency (`GF-FX`/`LDG-4.1`); (b) the split is computed and reconciled in the **native** currency (Σ 3 splits == **9000 GBP-minor exactly**, integer-exact, no float drift), NOT in converted USD — splitting the rounded $115.83 three ways would lose a cent, so the split MUST sum in GBP-minor and convert each share for display only; (c) the USD rollup of the whole purchase uses the **triangulated** rate once (≈ $115.83), and the per-share USD display sums back to that same rolled-up total within the documented ±1-minor-unit rounding tolerance — i.e. converting-then-summing and summing-then-converting agree to tolerance, and the residual cent has a deterministic owner (last share), never silently dropped or duplicated; (d) marking 2 of the 3 shares reimbursable creates exactly **2** GBP-denominated receivables (settled by GBP-equivalent inflows, matched in native amount), not 6 (the dedup members must not fan out); (e) a missing-rate day still triangulates or flags unconvertible (never silent 1:1), and the dedup group is unaffected by the FX gap; (f) `GF-REPLAY`: re-deriving preserves the single group, the GBP-stored splits, the receivables, and the cached triangulated rate with zero row delta. Proves dedup + FX-triangulation + split + reimburse compose without (i) double-counting via member fan-out, (ii) rounding loss from splitting a converted total, or (iii) a destructive currency rewrite. (Wired: LDG-4.1, LDG-4.2, ENR-3.1, ENR-4.1, ENR-4.3.)
>
...
> Closes iteration-05 self-critique #6 — flags now have a single registry like ENR-2.7 owns match-config. **PLT-5.3 owns this table as seeded data** (the `feature_flags` config); every flag-gated leaf reads its key from here, no ad-hoc flag strings. Default OFF = ships dark / behind verification.
...
|---------|--------------------|----------------|
| `GF-DEDUP` | 1 purchase via 3 sources → counted once | ENR-2.1/2.6/3.1, INT-4.2 |
...
| `GF-TRUESPEND` | true-spend = Σout − transfers − cardpay − excl − refunds | LDG-2.4, RPT-2.4/3.1, RPT-5.3 |
...
_End of v7. The final pass (v8) is the CONSOLIDATION pass — reconcile index blocks A–E against the leaves (regen, leaf wins), tighten any prose bloat, and do a full coverage+invariant sweep per the quality bar in 00-README.md §4. Leaf IDs are stable; splits keep the original ID and add new sibling numbers._
