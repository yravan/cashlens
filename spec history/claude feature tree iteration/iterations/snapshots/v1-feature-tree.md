...
Version: **v1** · See [00-README.md](00-README.md) for vision, conventions, legend, and the iteration protocol.
...
- `_deps:_` upstream leaf IDs that must land first.
- `_skill:_` repo/vendor skill an implementer invokes (see README §7).
- `_data:_` concrete schema/endpoint/shared-type delta (grounded in real `models.py`/`schemas.py`/`api-types`).
- `_accept:_` one-line acceptance criterion (the observable that proves it works).
- `_test:_` proving test layer — `pytest` (api), `vitest` (web unit), `playwright` (e2e), or `manual`/`doc`.
...
- `_risk:_` only on risky/ambiguous/spike leaves — risk + one-line mitigation.
...
> **Phase rollup:** P0 Foundation (PLT-2, SEC-1, LDG-4.3, AUT-1/2) · P1 Source+Ingest (SRC-1/2, ING-1/2/3) · P2 Ledger+Cat (LDG-1/2/3, CAT-1/3/4) · P3 Wedge (ENR, RCP, OWE) · P4 Intelligence+Reports (INT, RPT) · P5 Review+Nav+Notif (REV, NAV, NTF) · P6 iOS (IOS).
...
- **PLT-2.3 — Idempotent migration/backfill test harness** `[new]` `M` `data` — reusable fixture asserting re-run safety for data migrations.
  _skill:_ db-evolution · _accept:_ harness runs any backfill twice and asserts no row delta on 2nd run. _test:_ pytest. _xc:_ idem.
...
- **SEC-2.3 — Verify Neon at-rest encryption posture** `[new]` `S` `research` — confirm Neon encryption + backup encryption guarantees.
  _accept:_ written finding citing Neon at-rest + PITR encryption. _test:_ doc. _risk:_ assumption of encryption unverified; resolve via vendor doc/contract.
...
- **SEC-3.1 — Account deletion orchestration (export → disconnect → cascade)** `[new]` `M` `api` — orchestrate full erasure with confirmation + audit.
  _deps:_ SEC-3.5, SEC-3.2, SRC-2.5 · _data:_ deletes user + cascade across all 7 tables. _accept:_ post-deletion no rows for `user_id` anywhere and providers revoked. _test:_ pytest (cascade leaves zero rows). _xc:_ authz, idem.
...
- **SEC-3.2 — Full data export (JSON/CSV)** `[new]` `M` `api` — async export bundle of all user data.
  _deps:_ PLT-3.1 · _data:_ reads all user tables → zipped JSON+CSV to Blob. _accept:_ export contains every transaction/account/receipt for the user. _test:_ pytest (row-count parity vs db). _xc:_ authz.
...
- **SEC-4.1 — Audit log of sensitive actions** `[new]` `M` `data` — append-only log of deletes, exports, connection changes, step-up.
  _data:_ `audit_log` (user_id, action, entity, ip, at). _accept:_ each sensitive action writes one immutable audit row. _test:_ pytest. _xc:_ authz, obs.
...
- **SEC-5.1 — SOC2-readiness checklist (doc)** `[new]` `S` `research` — gap analysis vs SOC2 controls.
  _accept:_ checklist enumerating controls + current gaps. _test:_ doc.
- **SEC-5.2 — LLM data-handling policy (no-train, redaction)** `[new]` `S` `research` — vendor no-train terms + what's sent to LLM.
  _deps:_ CAT-4.1 · _accept:_ documented policy: zero-retention/no-train provider + redaction before send. _test:_ doc. _risk:_ provider terms may allow training; pin to enterprise no-train tier.
...
- **AUT-1.2 — Sign in with Google (Clerk OAuth)** `[new]` `S` `web` — Google as social provider.
  _deps:_ AUT-1.1 · _accept:_ Google button completes OAuth and creates/links the user. _test:_ playwright (mocked) + manual. _xc:_ authz.
...
- **AUT-2.2 — Sign-out + session revoke** `[partial]` `S` `web` — clear session + revoke at Clerk.
  _accept:_ after sign-out, protected routes redirect to login. _test:_ playwright. _xc:_ authz.
...
- **AUT-3.1 — User provisioning on first login (upsert by external id)** `[partial]` `M` `api` — idempotent upsert keyed on `external_auth_user_id`.
  _deps:_ SEC-1.1 · _data:_ upsert `users` by `external_auth_user_id`. _accept:_ repeated logins never create duplicate users. _test:_ pytest (double-login → 1 row). _xc:_ authz, idem.
...
- **AUT-4.1 — Active sessions list + revoke** `[new]` `M` `web` — list devices/sessions, revoke individually.
  _skill:_ clerk-cli · _accept:_ revoking a session logs that device out. _test:_ playwright + manual. _xc:_ authz.
- **AUT-4.2 — Step-up auth for sensitive actions** `[new]` `M` `web` — re-verify before delete/export/disconnect.
  _deps:_ AUT-2.1, SEC-3.1 · _accept:_ account-deletion requires fresh re-auth. _test:_ playwright. _xc:_ authz.

### AUT-5 Household / shared access
- **AUT-5.1 — SPIKE: data-sharing model (household vs per-user shares)** `[new]` `S` `research` — decide ownership/sharing model before building.
  _accept:_ recommendation: household tenant vs per-resource sharing, with authz implications. _test:_ doc. _risk:_ wrong model forces a painful re-scoping migration; decide before LDG/ENR mature.
- **AUT-5.2 — Household/workspace model + membership** `[new]` `M` `data` — group users into a shared workspace with roles.
  _deps:_ AUT-5.1, PLT-2.1 · _data:_ `households` + `household_members`(role); add `household_id` scoping path. _accept:_ a user can belong to a household with a role. _test:_ pytest. _xc:_ authz, idem.
- **AUT-5.3 — Invite/accept household member** `[new]` `M` `web` — invite by email, accept flow.
  _deps:_ AUT-5.2 · _accept:_ invitee accepts and gains scoped access. _test:_ playwright + pytest. _xc:_ authz.
- **AUT-5.4 — Shared-resource authorization (row scoping by household)** `[new]` `M` `api` — extend row-scoping to household membership.
  _deps:_ AUT-5.2, SEC-1.2 · _accept:_ a member sees shared data; a non-member is denied. _test:_ pytest (cross-household access denied). _xc:_ authz.
...
- **SRC-1.2 — Generalize `plaid_items` → `connections`** `[partial]` `M` `data` — rename/extend to provider-neutral `connections` with `provider` discriminator.
  _skill:_ db-evolution · _deps:_ PLT-2.1 · _data:_ migrate `plaid_items`→`connections` (+`provider`, keep `plaid_item_id` as provider_ref); update FKs + `PlaidItemRead`→`ConnectionRead`, `api-types`. _accept:_ existing Plaid rows readable post-migration; backward-compatible read. _test:_ pytest (migration upgrade/downgrade + data preserved). _xc:_ idem.
...
- **SRC-1.5 — SPIKE: bank provider choice (Plaid vs Teller vs SimpleFIN)** `[new]` `S` `research` — cost/coverage/data-quality comparison; recommend default.
  _accept:_ written recommendation with cost-per-item + coverage matrix. _test:_ doc. _risk:_ project memory says Teller default but repo is Plaid-only; treat as open decision, document migration cost either way.
...
- **SRC-2.2 — Public-token exchange + item store** `[built]` `M` `api` — `POST /plaid/exchange-public-token`; persist item + accounts.
  _deps:_ SEC-1.3 · _data:_ writes `plaid_items`(encrypted token)+`financial_accounts`; `ExchangePublicTokenResponse`. _accept:_ exchange stores encrypted token + creates accounts; re-exchange is idempotent. _test:_ pytest. _xc:_ crypto, idem, authz.
...
- **SRC-2.4 — Reconnect / Link update mode** `[new]` `M` `web` — re-auth an errored item without losing history.
  _data:_ link token in update mode for existing `plaid_item_id`. _accept:_ reconnect clears `needs_reauth` and resumes sync; history preserved. _test:_ playwright + pytest. _xc:_ idem, authz.
...
### SRC-3 Venmo
- **SRC-3.1 — SPIKE: Venmo data access (API vs statement export)** `[new]` `S` `research` — confirm no consumer API; statement CSV path.
  _accept:_ documented: Venmo has no public consumer txn API → CSV export is the path; note Plaid-via-PayPal limits. _test:_ doc. _risk:_ no official API; rely on user CSV upload, no auto-sync.
- **SRC-3.2 — Venmo statement/CSV parser** `[new]` `M` `data` — parse Venmo CSV schema → normalized rows.
  _deps:_ SRC-3.1 · _data:_ map CSV cols → raw event shape (amount minor units, date, counterparty, note). _accept:_ sample CSV parses to expected normalized rows incl. fees. _test:_ pytest (fixture CSVs). _xc:_ money, idem.
- **SRC-3.3 — Venmo → raw events ingestion** `[new]` `M` `data` — idempotent upsert of parsed Venmo rows.
  _deps:_ SRC-3.2, ING-4.1 · _data:_ insert into raw store with `provider='venmo'` + stable dedup key. _accept:_ re-uploading same CSV adds no duplicates. _test:_ pytest. _xc:_ idem, money.

### SRC-4 PayPal
- **SRC-4.1 — SPIKE: PayPal API access** `[new]` `S` `research` — evaluate PayPal Transaction Search API + scopes.
  _accept:_ documented feasibility of PayPal Transaction Search API (3y window, OAuth scopes) vs CSV fallback. _test:_ doc. _risk:_ API rate/window limits; fallback to statement import.
...
- **SRC-5.1 — SPIKE: investments access (Plaid Investments vs direct)** `[new]` `S` `research` — coverage for Fidelity/Robinhood/Kalshi.
  _accept:_ matrix of which providers cover each broker; Kalshi/Robinhood gaps noted. _test:_ doc. _risk:_ Kalshi/Robinhood lack reliable aggregation → manual entry fallback.
...
- **SRC-6.1 — Gmail OAuth connection** `[new]` `M` `api` — OAuth consent + read-only Gmail scope.
  _skill:_ env-vars · _deps:_ SEC-3.3 · _accept:_ user grants Gmail read scope and a connection is created. _test:_ pytest (mocked OAuth) + manual. _xc:_ authz, crypto. _risk:_ Google restricted-scope verification/CASA audit needed for prod; note timeline.
...
- **SRC-7.1 — SPIKE: SMS/iMessage receipt ingestion** `[new]` `S` `research` — feasibility + privacy of iMessage channel.
  _skill:_ imessage · _accept:_ documented approach + privacy constraints; likely macOS-only. _test:_ doc. _risk:_ no sanctioned iMessage API; prefer forward-to-email fallback.
...
- **SRC-8.1 — CSV import (mapping + preview)** `[new]` `M` `data` — generic CSV importer with column mapping + dedup preview.
  _deps:_ ING-4.1 · _data:_ raw rows `provider='csv'`, stable hash dedup key. _accept:_ user maps columns, previews, and import is idempotent. _test:_ pytest (fixture CSVs) + playwright (mapping UI). _xc:_ idem, money.
...
- **SRC-8.3 — Manual account (cash/asset/liability) entry** `[new]` `M` `web` — track cash/assets/liabilities not at any provider.
  _deps:_ LDG-3.1 · _data:_ `financial_accounts` row, `plaid_item_id=null`, manual type. _accept:_ manual account contributes to balances/net worth. _test:_ playwright + pytest. _xc:_ money, authz.
...
- **ING-1.1 — Paged initial transactions backfill** `[partial]` `M` `api` — page provider history into raw store on connect.
  _skill:_ db-evolution · _deps:_ ING-4.1 · _data:_ bulk upsert `raw_transactions`. _accept:_ connecting backfills full available window without dupes. _test:_ pytest (paged fixture). _xc:_ idem, rate.
...
- **ING-2.3 — Pending→posted transition handling** `[new]` `M` `data` — reconcile pending into posted via `pending_transaction_id`.
  _deps:_ ING-4.2 · _data:_ link `raw_transactions.pending_transaction_id`; supersede pending. _accept:_ a pending then posted pair yields one ledger event, not two. _test:_ pytest. _xc:_ idem, money.
...
- **ING-4.1 — Idempotent raw upsert (by provider txn id)** `[built]` `M` `data` — upsert keyed on provider transaction id.
  _data:_ unique `raw_transactions.plaid_transaction_id`. _accept:_ re-ingesting same txn updates in place, never duplicates. _test:_ pytest. _xc:_ idem.
- **ING-4.2 — Raw dedup (pending vs posted)** `[new]` `M` `data` — collapse pending+posted of one purchase.
  _deps:_ ING-4.1 · _data:_ dedup via `pending_transaction_id`. _accept:_ one logical purchase = one canonical raw row. _test:_ pytest. _xc:_ idem.
...
- **LDG-1.1 — Raw→ledger normalization service** `[built]` `M` `api` — derive a `LedgerEvent` from each raw transaction.
  _data:_ `raw_transactions`→`ledger_events`. _accept:_ every non-removed raw row yields exactly one ledger event. _test:_ pytest. _xc:_ idem, money.
- **LDG-1.2 — Idempotent ledger derivation (no dup events)** `[partial]` `M` `data` — re-derivation never doubles events.
  _skill:_ db-evolution · _deps:_ LDG-1.1 · _data:_ unique link `ledger_events.raw_transaction_id`. _accept:_ running derivation twice yields identical event set. _test:_ pytest. _xc:_ idem.
...
- **LDG-2.2 — Transfer candidate pairing (opposite amount/date window)** `[new]` `M` `data` — find candidate in/out pairs across accounts.
  _deps:_ LDG-2.1, LDG-4.3 · _data:_ candidate pairs keyed on equal minor-unit amount + near date. _accept:_ a $500 out + $500 in within window are flagged as a candidate pair. _test:_ pytest. _xc:_ money, idem.
- **LDG-2.6 — Transfer confirmation + flagging** `[new]` `M` `data` — promote confirmed pairs to `is_transfer=true` on both legs.
  _deps:_ LDG-2.2 · _data:_ sets `ledger_events.is_transfer` + pair link. _accept:_ confirming a pair excludes both legs from true-spend; unmatch reverts. _test:_ pytest. _xc:_ idem, money.
- **LDG-2.3 — Card-payment detection + exclusion** `[partial]` `M` `data` — detect credit-card payments (checking→card).
  _deps:_ LDG-2.2 · _data:_ `ledger_events.is_card_payment`. _accept:_ a detected card payment is excluded from spend. _test:_ pytest. _xc:_ money, idem.
- **LDG-2.4 — True-spend computation** `[built]` `M` `api` — spend excluding transfers/card payments/excluded.
  _deps:_ LDG-2.3, LDG-2.6 · _data:_ feeds `DashboardSummary.true_spend_this_month`. _accept:_ true-spend = sum(outflow) − transfers − card payments − excluded. _test:_ pytest (golden dataset). _xc:_ money.
- **LDG-2.5 — Manual exclude-from-spend override** `[built]` `S` `web` — user toggles `exclude_from_spend`.
  _data:_ `PATCH /transactions/{id}` `exclude_from_spend`. _accept:_ toggling immediately changes true-spend total. _test:_ playwright + pytest. _xc:_ money, authz.

### LDG-3 Balances & net worth
- **LDG-3.1 — Account balance store + history** `[partial]` `M` `data` — current + dated historical balances.
  _deps:_ LDG-4.3 · _data:_ `balance_history`(account_id, balance_minor, as_of); `financial_accounts.current_balance`. _accept:_ each sync records a dated balance row. _test:_ pytest. _xc:_ money, idem.
...
- **LDG-3.6 — Manual cash adjustment / reconcile entry** `[new]` `M` `web` — adjust a manual cash/asset account to a counted balance.
  _deps:_ SRC-8.3, LDG-3.5 · _data:_ writes an adjustment ledger event to true up balance. _accept:_ reconciling sets the manual account to the entered balance via an adjustment event. _test:_ playwright + pytest. _xc:_ money, idem, authz.
...
- **LDG-4.3 — Migrate money Float→integer minor units (schema+backfill)** `[new]` `M` `data` — add integer minor-unit columns + backfill from `Float`.
  _skill:_ db-evolution · _deps:_ PLT-2.1, PLT-2.3 · _data:_ add `amount_minor`(BigInt) to `ledger_events`/`raw_transactions`/`financial_accounts`; backfill `round(amount*100)`. _accept:_ backfill is exact and re-runnable; values match within rounding tolerance. _test:_ pytest (backfill idempotent + parity). _xc:_ money, idem. _risk:_ float→int rounding edge cases; assert tolerance + reconcile totals.
...
- **ENR-2.1 — Match candidate generation (blocking by amount+date)** `[new]` `M` `data` — generate candidate pairs across sources via blocking keys.
  _deps:_ LDG-4.3 · _data:_ candidate index on (amount_minor, date±window). _accept:_ given two sources of one purchase, the pair is a candidate. _test:_ pytest. _xc:_ money, idem.
- **ENR-2.6 — Match scoring + link model** `[new]` `M` `data` — score candidates (amount/date/merchant) + persist `match_links`.
  _deps:_ ENR-2.1 · _data:_ `match_links`(left_event, right_event, score, status). _accept:_ high-score pair auto-linked, mid-score queued, low-score dropped. _test:_ pytest (scoring thresholds). _xc:_ idem.
...
- **ENR-3.1 — Cross-source dedup model (canonical purchase + members)** `[new]` `M` `data` — one purchase = one canonical record with N source members.
  _skill:_ db-evolution · _deps:_ ENR-2.6 · _data:_ `purchase_groups` + member links; canonical chosen by precedence. _accept:_ bank+Venmo+receipt of one purchase roll up to a single canonical amount counted once. _test:_ pytest. _xc:_ idem, money.
- **ENR-3.4 — Dedup application to spend totals** `[new]` `M` `api` — ensure deduped purchases count once in spend/reports.
  _deps:_ ENR-3.1, LDG-2.4 · _accept:_ true-spend counts a deduped purchase exactly once. _test:_ pytest (golden multi-source dataset). _xc:_ money, idem.
...
- **ENR-4.1 — Split transaction model (parent + child splits)** `[new]` `M` `data` — model a transaction split into N parts summing to the whole.
  _deps:_ LDG-4.3 · _data:_ `transaction_splits`(parent_event, amount_minor, category); parent flagged split. _accept:_ splits must sum to parent amount; reject otherwise. _test:_ pytest (sum invariant). _xc:_ money, idem.
...
### RCP-2 Parsing / OCR
- **RCP-2.1 — SPIKE: OCR vendor (Textract vs GPT-4o vision vs Tesseract)** `[new]` `S` `research` — accuracy vs cost-per-receipt.
  _accept:_ recommendation with per-receipt cost + accuracy on a sample set. _test:_ doc. _risk:_ OCR cost at scale; favor vision-LLM only when rules fail.
...
- **RCP-2.5 — OCR text → structured header fields** `[new]` `M` `api` — parse merchant/total/tax/date from OCR text.
  _deps:_ RCP-2.2 · _skill:_ ai-sdk · _data:_ structured fields on `receipts`. _accept:_ OCR text yields merchant+total+date+tax with confidence. _test:_ pytest. _xc:_ money.
- **RCP-2.3 — LLM line-item extraction** `[new]` `M` `api` — structured per-line items from receipt text/image.
  _deps:_ RCP-2.2, RCP-3.1 · _skill:_ ai-sdk · _accept:_ extracted line items sum (±tax/tip) to receipt total. _test:_ pytest (structured-output schema + sum check). _xc:_ money. _risk:_ LLM hallucinated totals; validate sum invariant + flag mismatch.
...
- **RCP-3.1 — Line item model (product/service, qty, unit price)** `[new]` `M` `data` — itemized lines under a receipt.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `receipt_line_items`(receipt_id, desc, qty, unit_price_minor, total_minor). _accept:_ line items persist with integer money. _test:_ pytest. _xc:_ money, idem.
...
- **RCP-4.1 — Invoice model + lifecycle (received/paid/overdue)** `[new]` `M` `data` — invoices with due dates + status.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `invoices`(payee, amount_minor, due_date, status). _accept:_ invoice transitions received→paid/overdue correctly. _test:_ pytest. _xc:_ money, idem.
...
- **CAT-1.1 — Two-level taxonomy (groups → categories)** `[new]` `M` `data` — category groups → categories hierarchy.
  _deps:_ PLT-2.1 · _data:_ `category_groups`, `categories`(group_id, name, is_default); `ledger_events.category/subcategory` reference. _accept:_ categories nest under groups; every event maps to one category. _test:_ pytest. _xc:_ idem.
...
- **CAT-3.1 — Rule model (match → set category/flags)** `[new]` `M` `data` — conditions (merchant/amount/account) → actions (category, flags).
  _deps:_ PLT-2.1, CAT-1.1 · _data:_ `categorization_rules`(user_id, match_json, actions_json, priority). _accept:_ a rule persists and is user-scoped. _test:_ pytest. _xc:_ authz, idem.
...
- **CAT-4.4 — Categorization prompt + taxonomy grounding** `[new]` `M` `api` — prompt template injecting user taxonomy + examples (constrains CAT-4.1 to valid categories).
  _deps:_ CAT-4.1 · _skill:_ ai-sdk · _accept:_ prompt only offers existing categories; new ones not invented. _test:_ pytest (prompt snapshot + constraint). _xc:_ obs.
...
- **CAT-5.2 — Auto-suggest rule from repeated corrections** `[new]` `M` `api` — propose a rule after N same corrections.
  _deps:_ CAT-3.1, CAT-5.1 · _accept:_ 3 identical corrections suggest a rule the user can accept. _test:_ pytest. _xc:_ idem.
- **CAT-5.3 — Per-merchant remembered category** `[new]` `M` `data` — sticky last-confirmed category per merchant.
  _deps:_ ENR-1.3, CAT-5.1 · _accept:_ a new txn from a corrected merchant uses the remembered category. _test:_ pytest. _xc:_ idem.
...
- **OWE-1.1 — Reimbursement claim model (amount, by whom, status)** `[new]` `M` `data` — receivable tied to a transaction/split.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `receivables`(user_id, source_event_id, debtor, amount_minor, status). _accept:_ a receivable persists with open/settled status. _test:_ pytest. _xc:_ money, authz, idem.
- **OWE-1.2 — Mark transaction/split as reimbursable** `[new]` `M` `web` — flag a txn/split as owed.
  _deps:_ ENR-4.3, OWE-1.1 · _accept:_ marking creates a receivable and shows an owed badge. _test:_ playwright + pytest. _xc:_ money, a11y.
- **OWE-1.3 — Settle reimbursement (match incoming payment)** `[new]` `M` `api` — close receivable when matching inflow arrives.
  _deps:_ ENR-2.6, OWE-1.1 · _accept:_ an incoming payment matching the amount settles the receivable. _test:_ pytest. _xc:_ money, idem.
...
- **OWE-2.1 — Group expense model (I paid, owed by N)** `[new]` `M` `data` — one expense split across participants.
  _deps:_ OWE-1.1 · _data:_ `group_expenses` + `participant_shares`. _accept:_ shares sum to the expense total. _test:_ pytest (sum invariant). _xc:_ money, idem.
...
- **OWE-3.1 — Return-intent model (item, expected refund)** `[new]` `M` `data` — track purchases intended for return.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `returns`(source_event_id, expected_refund_minor, status). _accept:_ a return-intent persists with expected refund. _test:_ pytest. _xc:_ money, idem.
...
- **OWE-4.1 — Scheduled/future transaction model** `[new]` `M` `data` — known upcoming inflows/outflows.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `scheduled_transactions`(amount_minor, date, direction, source). _accept:_ scheduled items persist and are queryable by date. _test:_ pytest. _xc:_ money, idem.
...
- **OWE-5.1 — Bill model (payee, amount, cadence, due)** `[new]` `M` `data` — recurring obligations with due dates.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `bills`(payee, amount_minor, cadence, next_due). _accept:_ a bill persists with cadence + next due date. _test:_ pytest. _xc:_ money, idem.
...
- **INT-1.1 — Recurring grouping (cluster by merchant+amount)** `[new]` `M` `api` — cluster historical events into candidate streams.
  _deps:_ ENR-1.3, INT-1.2 · _accept:_ Netflix monthly charges cluster into one candidate stream. _test:_ pytest (fixture history). _xc:_ idem.
- **INT-1.5 — Cadence inference + next-date prediction** `[new]` `M` `api` — infer interval (weekly/monthly/annual) + predict next date.
  _deps:_ INT-1.1 · _accept:_ a monthly stream predicts the next charge date within tolerance. _test:_ pytest. _xc:_ idem.
...
- **INT-4.1 — Anomaly baseline + scoring** `[new]` `M` `api` — per-merchant/category baselines (mean/stdev) + z-score.
  _deps:_ ENR-1.3 · _accept:_ a 3x-typical charge scores as anomalous. _test:_ pytest (fixture distribution). _xc:_ money, idem.
- **INT-4.5 — Anomaly surfacing + dismiss** `[new]` `M` `api` — turn high scores into reviewable anomaly items.
  _deps:_ INT-4.1 · _data:_ `anomalies` records. _accept:_ anomalous charge appears as an anomaly the user can dismiss. _test:_ pytest. _xc:_ idem.
- **INT-4.2 — Duplicate-charge detection** `[new]` `M` `api` — same merchant+amount within a short window.
  _deps:_ ENR-3.1 · _accept:_ a double charge is flagged (and not confused with deduped multi-source). _test:_ pytest. _xc:_ money, idem.
- **INT-4.3 — Price-change detection on recurring** `[new]` `M` `api` — detect recurring amount increases.
  _deps:_ INT-1.2 · _accept:_ a subscription price hike is flagged with old→new amount. _test:_ pytest. _xc:_ money.
...
- **RPT-2.4 — Aggregation query API (group by period/category)** `[new]` `M` `api` — reusable rollup endpoint powering charts.
  _deps:_ LDG-4.4 · _data:_ `GET /reports/aggregate?by=period|category` + shared type. _accept:_ returns summed minor-unit amounts grouped as requested, true-spend aware. _test:_ pytest (golden dataset). _xc:_ money, authz.
...
- **RPT-7.3 — Savings goal model (target, deadline, linked account)** `[new]` `M` `data` — goals toward a target amount.
  _deps:_ PLT-2.1, LDG-4.3 · _data:_ `goals`(name, target_minor, deadline, account_id). _accept:_ a goal persists with target + progress source. _test:_ pytest. _xc:_ money, idem.
...
- **RPT-7.1 — Saved/bookmarked report views** `[new]` `M` `web` — save report+filter configs.
  _deps:_ NAV-3.2 · _accept:_ a saved view reloads with its filters intact. _test:_ playwright. _xc:_ authz.
- **RPT-7.2 — Export report (CSV/PDF)** `[new]` `M` `web` — export current report.
  _deps:_ RPT-2.4 · _accept:_ exported CSV totals match the on-screen report. _test:_ playwright + pytest. _xc:_ money.
...
- **REV-1.1 — Queue builder (uncategorized/low-confidence/anomalies)** `[new]` `M` `api` — assemble items needing attention.
  _deps:_ LDG-1.1 · _data:_ query over `ledger_events` (uncategorized, `confidence<θ`) + anomalies. _accept:_ queue contains uncategorized + low-confidence + flagged items. _test:_ pytest. _xc:_ authz.
...
- **REV-4.1 — Bulk review (apply to similar)** `[new]` `M` `web` — "apply to all like this" from a card.
  _deps:_ CAT-3.3, REV-2.5 · _accept:_ resolving with "apply to similar" clears matching queue items. _test:_ playwright. _xc:_ idem.
...
- **NAV-3.1 — Transaction filter API (multi-dimension)** `[partial]` `M` `api` — server-side filtering (date/amount/category/account/type/flags/merchant).
  _data:_ extend `GET /transactions` query params + indexes. _accept:_ each documented filter param narrows results correctly. _test:_ pytest (per-dimension). _xc:_ authz, money.
- **NAV-3.4 — Transaction filter UI (chips + panel)** `[new]` `M` `web` — filter chips + panel bound to URL state.
  _deps:_ NAV-3.1, NAV-3.2 · _accept:_ applying filters updates the list and the URL. _test:_ playwright. _xc:_ a11y.
- **NAV-3.2 — Saved filter views (URL state via nuqs)** `[new]` `M` `web` — shareable filter state in URL + saved views.
  _deps:_ NAV-3.1 · _accept:_ a filtered URL reproduces the same view on load. _test:_ playwright. _xc:_ authz.
...
- **NTF-1.2 — Mark read / read-all** `[built]` `S` `web` — mark individual/all read.
  _data:_ `PATCH /notifications/{id}/read`, `PATCH /notifications/read-all`. _accept:_ marking read clears the unread badge. _test:_ playwright + pytest. _xc:_ authz.
...
- **NTF-3.1 — Service worker + push subscription (VAPID)** `[new]` `M` `web` — register SW + subscribe with VAPID.
  _accept:_ granting permission stores a push subscription server-side. _test:_ manual + vitest (subscribe logic). _xc:_ authz, crypto.
- **NTF-3.3 — Push subscription storage + lifecycle** `[new]` `S` `api` — persist/expire subscriptions.
  _deps:_ NTF-3.1, PLT-2.1 · _data:_ `push_subscriptions`(user_id, endpoint, keys). _accept:_ expired/invalid subscriptions are pruned on send failure. _test:_ pytest. _xc:_ authz, idem.
...
- **NTF-5.2 — Digest builder (daily/weekly)** `[new]` `M` `api` — batch deferred notifications into digests.
  _deps:_ NTF-2.2, PLT-3.1 · _accept:_ deferred items aggregate into one digest at cadence. _test:_ pytest. _xc:_ idem, obs.
- **NTF-5.3 — Event triggers (new txn, anomaly, bill due)** `[partial]` `M` `api` — central trigger → notification creation.
  _data:_ writes `notification_events`. _accept:_ each trigger type creates the right notification once. _test:_ pytest. _xc:_ idem, obs.
...
- **IOS-1.1 — SPIKE: iOS approach (React Native/Expo vs Swift vs PWA)** `[new]` `S` `research` — pick the stack.
  _accept:_ recommendation weighing code-reuse vs native (push, biometrics, share-ext). _test:_ doc. _risk:_ stack choice gates all IOS work; decide before IOS-1.2.
...
- **IOS-1.3 — Clerk auth on iOS** `[new]` `M` `ios` — native sign-in via Clerk.
  _skill:_ clerk-cli · _deps:_ IOS-1.4 · _accept:_ user signs in and reaches the dashboard tab. _test:_ manual. _xc:_ authz.
...
- **IOS-2.3 — Review queue (native swipe)** `[new]` `M` `ios` — native swipe review.
  _deps:_ IOS-1.5, REV-1.3 · _accept:_ swipe categorizes and advances the queue. _test:_ manual. _xc:_ a11y.
...
- **IOS-3.1 — Push notifications (APNs)** `[new]` `M` `ios` — receive + deep-link pushes.
  _deps:_ NTF-4.1 · _accept:_ a push opens the relevant screen. _test:_ manual. _xc:_ authz.
- **IOS-3.2 — Biometric app lock (FaceID)** `[new]` `M` `ios` — FaceID gate on launch.
  _accept:_ app requires biometric unlock when enabled. _test:_ manual. _xc:_ authz.
- **IOS-3.3 — Share extension (save receipt to app)** `[new]` `M` `ios` — share a receipt image into the app.
  _deps:_ RCP-1.1 · _accept:_ sharing an image creates a receipt record. _test:_ manual. _xc:_ authz.
- **IOS-3.4 — Offline read-only cache** `[new]` `M` `ios` — cache recent data for offline viewing.
  _deps:_ IOS-1.4 · _accept:_ recent dashboard/txns viewable offline; mutations queue or block. _test:_ manual. _xc:_ idem.
...
_End of v1. Subsequent passes (v2–v8) continue refining the whole tree per the quality bar in 00-README.md §4. Leaf IDs are stable; splits keep the original ID and add new sibling numbers._
