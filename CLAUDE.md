...
- CI (leaf 10.1): `.github/workflows/ci.yml` runs `lint-typecheck` and `e2e` (production build + the full Playwright suite against compose Postgres and the real Clerk dev instance) on every PR and on `main`; both are required status checks on `main`, so red blocks merge. The repo is public: Actions logs and artifacts are world-readable. Failure artifacts are scrubbed of Clerk tokens by `e2e/scrub-secrets.ts` (Playwright `globalTeardown`; CI refuses to upload without its `.scrubbed` stamp). Never enable `DEBUG=pw:api` in CI and never assert with `toBeOK()` (lint-enforced) — both print live session cookies into logs.
