...
- CI (leaf 10.1): `.github/workflows/ci.yml` runs `lint-typecheck` and `e2e` (production build + the full Playwright suite against a real Postgres 16 — service container in CI, compose locally — and the real Clerk dev instance) on every PR and on `main`; both are required status checks on `main`, so red blocks merge. The e2e job runs in the pinned `mcr.microsoft.com/playwright` image (keep its tag in lockstep with `@playwright/test`): browsers and system deps are baked in, so no apt/CDN on the hot path. The repo is public: Actions logs and artifacts are world-readable. Failure artifacts are scrubbed of Clerk tokens by `e2e/scrub-secrets.ts` (Playwright `globalTeardown`; CI refuses to upload without its `.scrubbed` stamp). Never enable `DEBUG=pw:api` in CI and never assert with `toBeOK()` (lint-enforced) — both print live session cookies into logs.
