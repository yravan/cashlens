...
- History backfill (leaf 2.1.3): `lib/data/plaid-sync.ts` pages Plaid `/transactions/sync` (500/page, ≤20 pages per attempt; a mutation error restarts the attempt from the run's origin cursor, ≤2 restarts, so a run advances ≤20 pages but may spend ≤60 calls) and commits mapped `added` rows + `connections.sync_cursor` + `backfill_status` atomically per run — idempotent on the `(account_id, source, source_id)` key, so re-runs and resumes never duplicate. Sign convention lands here: Plaid positive = money out; every amount is negated at ingestion into the ledger's net-worth convention. Balances refresh via `/accounts/get` when history completes (sync responses list only accounts with transactions in that page). Trigger: connect-button polls `POST /api/connections/:id/sync` until `backfill_status='complete'`; a closed tab parks the import at the stored cursor. Seams: 2.1.4 resumes continuous sync from that same cursor (the backfill's first sync call also arms `SYNC_UPDATES_AVAILABLE` webhooks) and owns applying `modified`/`removed`, which backfill deliberately drops — a pending that posts mid-backfill still lands via its posted `added` twin; the leftover pending row is 3.2.1's reconciliation input.
