...
- Seeded data (leaf 10.4): one canonical deterministic dataset in `apps/web/db/seed/dataset.ts` — personas `demo` (5 accounts, 16 transactions, USD+EUR, every source and status, two zero-sum transfer pairs), `neighbor` (1 account/2 transactions, the isolation counterpart), `empty` (0/0) — with expected aggregates derived from the rows and exported as `EXPECTED` (hand-verified anchors pinned in `tests/api/seed-dataset.test.ts`). Tests assert against `EXPECTED`, never hand-recounted literals. Consumption: api tests call `seedDataset(adminDb())` (optionally mapping personas onto provisioned users); e2e calls `seedLedgerFixture({ demo, neighbor })` to map personas onto the two real Clerk e2e users; `pnpm db:seed [clerkUserId]` fills the local dev database — "demos without real credentials" today means fake ledger data under a Clerk dev-instance sign-in (attach the demo ledger to your own user to browse it); no real financial data or provider credentials anywhere. `db:seed` fails closed on any non-loopback database URL (`db/seed/local-only.ts`, guard is tested). No frozen-clock helper yet — nothing under test reads the clock; dataset dates are fixed absolutes. Transaction signs follow the ledger's net-worth convention (Plaid's is the inverse; negate at ingestion when 2.1.x lands).
